...
**Status: CANDIDATE — NOT ADOPTED. NOT CRC-ELIGIBLE. NOT READY FOR FGR (see §17 gate below — one load-bearing gap is currently open).** This document is a pre-adoption governance-drafting artifact only. It does not modify `GOVERNED-CLAIMS.md`, `TOPIC-RELATIONSHIPS.md`, `PENDING-QUESTIONS.md`, `EDGE-CASES.md`, `SI8-POSITIONS.md`, any production `TopicClaim`/`TopicRelationship` fixture, or any `geographic_relevance_scope` production value. Article 50(4) production onboarding remains PAUSED.
...
**Not captured, and material to this package (see §Open Issues for full list):** Article 3's definitions — specifically "deep fake," "deployer," and "provider" — were never independently captured as primary text in any of the three evidence sessions. This package uses those terms exactly as they appear in the Article 50(4) text itself (which does use "deployer" and "deep fake" without redefining them inline) but cannot state their precise statutory boundary conditions without that capture. **This is treated as a load-bearing evidence gap, not filled by recollection or inference — see §17 Q1.**
...
**Topic: [OPEN — see §8. No existing `GoalCategory` value is a clean fit. This field cannot be finalized without a taxonomy decision outside this milestone's authority. Provisionally left blank rather than forced into `likeness`, which would misrepresent the proposition's subject — see §8's full reasoning.]**
...
**SI8 interpretation:** [SI8 operational judgment, not itself sourced from the statute] A commercially distributed AI video containing synthetic audiovisual content that a reasonable viewer could mistake for authentic (i.e., plausibly "constituting a deep fake," pending the Article 3 definition gap in §Open Issues) should not be represented to a client, buyer, or platform as clear of Article 50(4) disclosure obligations without confirming both (a) whether the content in fact constitutes a "deep fake" under the Regulation's own definition, and (b) who the "deployer" is for this specific project — SI8's typical client role (production agency creating content on behalf of a brand/publisher) does not automatically resolve either question, and CRC must not assume it does.
...
## 8. Explicit-goal authorization / TopicRelationship (Step 8) — BLOCKED, reported per instruction

Per Step 8's own explicit instruction ("If no current explicit goal cleanly authorizes contribution: STOP that portion of activation design. Report the taxonomy/governance gap. Do not manufacture a relationship merely to activate geographic discovery"), **this step is stopped, not completed.**
...
|---|---|---|---|---|
| `deployer_status_confirmed` (is the user/their client the "deployer" for this specific AI system's output, per Article 3's undefined-in-this-package sense of the term?) | **D — not currently representable.** No structured fact type exists anywhere in the reviewed interview-engine contract for this concept. | The entire obligation is deployer-specific; without this, CRC cannot even identify who the claim addresses. | No — nothing to ask against; would require new engineering, out of scope here. | Yes — absent this, the claim cannot resolve to "applies to you" for any project. |
| `content_constitutes_deep_fake` (does the specific content meet the Regulation's own "deep fake" definition?) | **C — evidence-only**, pending §Open Issues' Article 3 capture. Even once the definition is captured, this looks like a judgment call closer to `advertising_or_trade_use_confirmed` in the NY likeness package (a legal characterization, not a raw fact) than a simple self-attestable yes/no. | Central gating condition for the whole obligation. | Provisionally no — mirrors the Stock Governance Rule (§ below): a legal-characterization question should not become a naive self-attestation question. | Yes. |
...
**Recommendation: `CRC eligible: Pending` — not `No`, but not ready for `Yes` either.** The proposition itself is CRC-suitable in principle. What blocks a `Yes` recommendation is entirely the taxonomy gap in §8 (no topic to attach it to means the question is currently moot) and the Article 3 definitional gap (§Open Issues) that leaves `content_constitutes_deep_fake` too imprecisely bounded to draft confident CRC-facing wording around yet.
...
**No load-bearing issue found that could be silently resolved without evidence.** The two real, unresolved issues (§8 taxonomy gap; §Open Issues Article 3 definitional gap) are marked OPEN below, not force-closed.
...
2. **[MATERIAL, not fully blocking] Article 3's "deep fake" and "deployer" definitions have never been independently captured as primary text** in any of this project's three EU AI Act evidence sessions. This package uses both terms exactly as Article 50(4) itself uses them, without redefining them, but cannot state their precise boundary conditions. A future bounded evidence-capture pass (mirroring the method that succeeded for Article 50/Article 2/Article 111) should target Article 3 specifically before this claim advances toward FGR.
