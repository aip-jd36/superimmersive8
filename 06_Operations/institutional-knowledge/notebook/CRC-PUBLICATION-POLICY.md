...
**7. CRC eligibility and Canonicalization Readiness are independent judgments (LK-93/LK-94, 2026-09-01).** `CRC-Eligible` concerns whether a specific governed proposition is approved for the unsupervised CRC channel — a content-safety/accuracy judgment, entirely independent of whether ordinary language currently resolves to the canonical tool/provider identity the proposition references. A claim may legitimately be `CRC Eligible: Yes` before its associated identity is conversationally runtime-ready. This is a codification of precedent already applied consistently across four CPR reviews (`CPR_007` §3, `CPR_009` §12, `CPR_010`, `CPR_011`) — not a new posture. `CPR_007`'s own §3 finding, quoted verbatim by the later three: *"The architecture itself correctly separates governance-layer eligibility from runtime reachability... This is a positive finding, not an architecture warning."* (`CPR_007`'s WITHHOLD disposition for the 10 Music Scenario A claims was driven by a separate prerequisite — empirical Retrieval/BI/Composition pipeline verification, unavailable before the synthetic-eligibility-canary tool existed — never by extraction/reachability coverage itself; see `governance-reviews/CPR_007_MUSIC_SCENARIO_A_PACKAGE_2026-08-27.md` §3 for the historical record, preserved verbatim and not rewritten here.)

Two further distinctions this principle preserves, never collapsed into one status:

- **Canonicalization Readiness** — a NEW, identity-level (not claim-level, not row-level) operational prerequisite, effective for any canonical tool/provider identity registered from 2026-09-01 onward: the real, unmodified canonicalization function must resolve at least one authoritative representative expression to that identity before it is considered conversationally runtime-ready and before production UAT for that identity is authorized. This does **not** gate `CRC-Eligible` itself (see above), and it does **not** apply retroactively — identities registered before 2026-09-01 are grandfathered as Canonicalization Readiness: UNKNOWN / NOT YET EVALUATED, an epistemic status, not a failure, and are not edited or backfilled by this principle. Implementation and enforcement: `08_Platform/app/lib/crc-engine/canonicalization-readiness.ts`.
- **Conversational extraction coverage** — whether ordinary user language causes the live extraction/model layer to propose the identity as a candidate at all. Remains a separate, disclosed limitation under current evidence (every confirmed production reachability failure to date — Storyblocks, Pond5, Adobe Stock, Luma — was a canonicalization-table defect, not an extraction-proposal defect) and is explicitly **not** established by Canonicalization Readiness passing. Not a mandatory gate; production UAT remains the operative check, consistent with existing precedent.

Governed-knowledge reachability (Retrieval/BI/Composition correctness) remains separately required under existing CPR review and the synthetic-eligibility-canary practice — unchanged by this principle.
