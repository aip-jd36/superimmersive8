...
**Status:** Accepted (design) — `ARCHITECTURE FROZEN / NOT IMPLEMENTED` (frozen 2026-09-10). Formalized + frozen with CAH-4G (`PRD_CAH_4G_HRR.md`, `HRR_GRI_TECHNICAL_DESIGN.md §S–§V`). No runtime code exists yet; this ADR states the durable principle the eventual implementation and every future channel must be checked against.
**Date:** 2026-09-10
...
**Frozen by `HRR_GRI_TECHNICAL_DESIGN.md §S–§T` (2026-09-10), not re-decided here:** audit = reuse `access_kind='lk_research'` + additive nullable enrichment, raw question never persisted (§T-1); `BiIntent`/`BiResult` generalization ships, HRR-local rule application rejected (§T-2); V1 composition fully deterministic (§T-3); mixed judgment+research → decline + research explicit portion (§T-5); likeness gap does not block V1 (§T-7); classifier inherits the interview-engine adapter config pattern (§T-8); `is_admin` OK for internal pilot, dedicated grant before broader rollout (§T-9).
