...
export async function runTurn(input: RunTurnInput, deps: RunTurnDeps): Promise<TurnOutcome> {
  ...
  if (declineSignal) {
    ...
    // Deterministic, no LLM call, no retrieve() call -- see
    // selector-questioning.ts's own header for why it calls
    // lookupTopicClaims() directly rather than requiring a full Retrieval
    // pass mid-turn. Computed LAST among the four forced-candidate sources
    // (after jurisdiction/human-contribution/Discovery), so it cannot alter
    // any existing precedence for those three, and so it can never fire for
    // a fact that jurisdiction's own dedicated module already owns
    // (selector-questioning.ts's own HANDLED_BY_DEDICATED_MODULE guard is
    // the actual enforcement; this ordering is additionally the smallest,
    ...
    const selectorNeeds = deriveSelectorNeeds(suAfter, topicClaims, boundaryStateForTurn)
