/**
 * CRC project-context projection (CAH-4B — extracted verbatim from
 * `lib/crc-sales/projection.ts`'s `buildSalesSessionProject`, CAH-3B).
 *
 * Pure. Deterministic. Presentation-only. Consumes ONLY the persisted
 * `StructuredUnderstanding` (already deserialized). It NEVER:
 *   - calls an LLM;
 *   - recomputes Retrieval or Bounded Interpretation;
 ...
 *
 * This body is behaviorally identical to the pre-CAH-4B
 * `buildSalesSessionProject` — `lib/crc-sales/projection.ts` now re-exports
 * this function unchanged, and an equivalence test asserts identical output.
 */
...
import type { CrcProjectContext, CrcProjectGoal, CrcProjectAssertion, CrcCorrectionHistoryItem } from './types'
...
function classify(
  ...
  current: CrcProjectAssertion[],
  history: CrcCorrectionHistoryItem[],
): void {
...
export function buildCrcProjectContext(su: StructuredUnderstanding): CrcProjectContext {
  const assertions: CrcProjectAssertion[] = []
  const correction_history: CrcCorrectionHistoryItem[] = []

  // Goals -- current vs superseded.
  const goals: CrcProjectGoal[] = []
