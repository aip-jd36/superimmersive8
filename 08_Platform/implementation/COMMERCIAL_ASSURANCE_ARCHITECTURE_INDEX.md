...
| **HRR (Human Reviewer Research) + GRI (Governed Research Interface) — CAH-4G** | `PRD_CAH_4G_HRR.md` (the *what*), `HRR_GRI_TECHNICAL_DESIGN.md` (the *how*), `ADR-002-governed-research-interface.md` (durable decision: a natural-language research interface is intent entry into governed retrieval + Bounded Interpretation, not an answer authority) | **`DESIGNED / NOT IMPLEMENTED`** — no runtime code. Design reuses `lib/reviewer-lk/**` (retrieval/eligibility/applicability), `lib/bounded-interpretation/**` (semantic ceiling; `determination_declined` = the assessment-authority refusal), the `anthropic-extractor.ts` structured-output pattern (one bounded classify-only model call for free-form). No CRC change. |
...
| **CAH-4G** | **HRR V1 / Governed Research Interface (GRI)** — free-form reviewer research questions alongside the existing topic look-up, inside the same inspector. Topic path unchanged (0 model calls). Free-form path = 1 bounded structured **classify-only** model call → governed retrieval (reuses `selectReviewerClaims`) → deterministic applicability → Bounded Interpretation (`determination_declined` handles assessment-authority questions) → deterministic consultative composition → provenance. Single-turn; no auto CRC context; no synthetic `UserGoal`s; no invented "closest topic"; reuse `access_kind='lk_research'`. **No CRC change.** | **`DESIGNED / NOT IMPLEMENTED`** (2026-09-10). Spec: `PRD_CAH_4G_HRR.md`; design: `HRR_GRI_TECHNICAL_DESIGN.md`; decision: `ADR-002-governed-research-interface.md`. Supersedes the intent-only boundary in `PRD_CAH_4F_REVIEWER_RESOURCES.md §11`. Awaiting PM architecture review → implementation authorization. |
