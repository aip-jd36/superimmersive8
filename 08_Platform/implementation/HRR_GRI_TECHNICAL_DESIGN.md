...
**Status:** `ARCHITECTURE FROZEN / IMPLEMENTATION IN PROGRESS` — **Slices 1–5 COMPLETE** (CAH-4G.1 / .2 / .3+3A / .4 / .5, 2026-09-10); **Slice 6 (Reviewer Resources HRR interface + audited runtime integration) COMPLETE (CAH-4G.6, 2026-09-10)**; Slice 7 not started. Sections A–R are the design narrative; **§S (Architecture Freeze), §T (Resolved open questions), §U (Implementation contract), §V (Frozen UAT)** are the binding implementation contract. **The Living Knowledge tab is now the converged Governed Research Interface: topic shortcuts + Ask HRR both POST `/api/admin/submissions/[id]/reviewer-lk/research`, which runs `runAuditedHrrResearch()` (audit-before-content, fail-closed) and returns one `HrrResearchAnswer` rendered by one `<HrrResearchAnswerView>`. Single-turn (no conversation memory); the raw question is never persisted. The legacy `GET .../reviewer-lk` topic route is retained as non-UI legacy pending a Slice-7 retirement. CRC behavior is unchanged. Not deployed — PM review pending.**
...
HrrAuthorityGateResult   (free-form: classifier → hrrAuthorityGate ;  topic chip: topicSelectionGateResult())
  ...
  → route (both entry modes)   POST app/api/admin/submissions/[id]/reviewer-lk/research/route.ts   (§A/§E)          [SHIPPED CAH-4G.6]
  → UI (topic chips + Ask HRR)  ReviewerLkLookup.tsx → one <HrrResearchAnswerView>                                  [SHIPPED CAH-4G.6]
```
...
| # | Frozen decision |
|---|---|
| S-1 | **Converged pipeline — SHIPPED CAH-4G.6.** Both entry modes produce `HrrResearchAnswer` via one internal `runHrrResearch()` → `projectHrrResearchAnswer()`, rendered by one `<HrrResearchAnswerView>`. The **one UI-reachable route** is `POST /api/admin/submissions/[id]/reviewer-lk/research` with `{ mode: 'topic_pick', topic }` \| `{ mode: 'question', question }`; both call `runAuditedHrrResearch(...)`. The CAH-4E `GET .../reviewer-lk?topic=` route + its `ReviewerLkLookupResult` shape are **superseded for the UI** but **retained as non-UI legacy** (its `route-and-audit.test.ts` + firewall refs + `projectReviewerLkResult` retire together — a Slice-7 cleanup). The CAH-4E `presentation` test was rewritten for `HrrResearchAnswerView`. |
...
| LEGACY (retained, non-UI) | `GET .../reviewer-lk?topic=` (CAH-4E topic look-up) — no UI consumer after CAH-4G.6; a Slice-7 retirement (its `route-and-audit.test.ts` + 3 firewall refs + `projectReviewerLkResult` / `ReviewerLkLookupResult` come with it) | `app/api/admin/submissions/[id]/reviewer-lk/route.ts` | retained pending Slice 7 |
