...
HrrAuthorityGateResult   (free-form: classifier → hrrAuthorityGate ;  topic chip: topicSelectionGateResult())
  ...
  → deterministic applicability   evaluateApplicabilityDetailed()           (inside the selector — REUSED)                     [SHIPPED]
       partition: any not_met → does_not_apply[] (NOT fed to BI) ;  met/unresolved → fed to BI
...
**SHIPPED (CAH-4G.3) — the BI partition (`lib/hrr/run-hrr-research.ts`):** for each researched topic, `runHrrResearch` partitions `selectReviewerClaims().claims` by applicability:
...
- **only `met` / `unresolved`** → the claim → `reviewerClaimToBiResult` → BI's `results`. A reviewer sees an `unresolved` requirement verbatim next to the claim; BI renders it (`directly_relevant` / Case 3B) because a professional reviewer, unlike an unsupervised CRC user, sees the full applicability picture.

This is the exact §H frozen behavior; it was **verified against `evaluateApplicabilityDetailed` source** (a `not_met` comes only from an explicitly-excluded jurisdiction / a confirmed non-matching tool fact — never from silence) before implementing.

## H. Bounded Interpretation — the smallest honest adaptation boundary

**Current (post-CAH-4G.3, 2026-09-10):** `buildBoundedInterpretations(intents: BiIntent[], results: BiResult[], diagnostics?, humanContributionDescription?)` (`build-bounded-interpretation.ts`). It reads only `intent.{intent_id, intent_text, category, scope}` and, per result, `{matched_goal_category, unresolved_project_dependencies, claim_id, candidate_statement, match_origin, source_fact.kind}` — the exact `BiResult` shape. A CRC `RetrievalResult[]` structurally satisfies `BiResult[]`, so CRC call sites are unchanged.
...
**Second adaptation boundary — results: `BiResult` INTRODUCED (CAH-4G.3, 2026-09-10).** Slice 1 deferred this "until a real consumer exists"; `runHrrResearch` is that consumer. `BiResult` = exactly the 6 fields `build-bounded-interpretation.ts` reads off a governed result: `matched_goal_category`, `unresolved_project_dependencies`, `claim_id`, `candidate_statement`, `match_origin` (only ever `=== 'related_topic'`), `source_fact.kind` (only ever `=== 'tool'`). **`source_fact` is kept NESTED (`{ kind }`)** — so a CRC `RetrievalResult` (which has all 6 fields + more) structurally satisfies `BiResult[]` and **every CRC call site + BI test is byte-unchanged** (proven: identical 20-failing-suite / 77-failing-test set before/after the type swap alone; the Slice-1 churn concern is fully resolved). HRR builds `BiResult` directly via **`reviewerClaimToBiResult(claim: ReviewerLkClaim): BiResult`** (`lib/hrr/bi-adapters.ts`): `matched_goal_category = claim.topic` (the selector only ever returns claims whose topic equals the researched topic — accurate, not fabricated), `candidate_statement = claim.statement` (verbatim governed `crc_candidate_statement`), `claim_id` / `unresolved_project_dependencies` verbatim, `match_origin = 'exact_topic'` (HRR has no related/discovered path), `source_fact = { kind: 'topic' }` (a topic-sourced non-tool claim). **It constructs NONE of `RetrievalResult`'s CRC-shaped fields** (`matrix_identifier`, `relationship_id`, `publication_scope`, `topic`, `source_fact.identifier`) — no synthetic value anywhere. `RetrievalResult` is removed from BI's public contract (`build-bounded-interpretation.ts` now imports only `BiIntent` / `BiResult` / `RetrievalDiagnostic`).

**Frozen BI-input semantics for HRR — SHIPPED CAH-4G.3 (`lib/hrr/run-hrr-research.ts`):**
- **`diagnostics` is always `[]`** for HRR (`buildBoundedInterpretations([biIntent], biResults, [], {state:'unknown'})`). `selectReviewerClaims` *surfaces* a topic-matched claim regardless of applicability status (it never produces an `applicability_unmet` diagnostic). So **Case 3A (formal-gate-unmet → claim withheld) never fires for HRR** — a claim with an `unresolved` applicability requirement still reaches BI's `matches[]` (via `reviewerClaimToBiResult`), renders as `directly_relevant` (or Case 3B), and the `unresolved` requirement is carried verbatim alongside in `HrrResearchTopicResult.governed_claims[].applicability_outcomes` ("not a negative finding"). Case 3B (`relevant_applicability_unresolved` via `unresolved_project_dependencies.length > 0`) applies and is wanted.
...
| S-5 | **BI adaptation** (§H — SHIPPED across CAH-4G.1/.2/.3): `buildBoundedInterpretations(BiIntent[], BiResult[], diagnostics?, humanContributionDescription?)`. **`BiIntent`** (Slice 1) — CRC adapts via `userGoalsToBiIntents` (1-line call-site change, **zero behavior change, proven**); BI no longer applies a lifecycle filter itself. **`BiResult`** (Slice 3) — the minimal 6-field result contract, `source_fact` kept **nested** so a CRC `RetrievalResult` structurally satisfies `BiResult[]` (**every CRC call site + BI test byte-unchanged, proven byte-for-byte behavior-neutral**); HRR builds it via `reviewerClaimToBiResult` (`lib/hrr/`) with **no synthetic field**. `RetrievalResult` removed from BI's public contract. HRR intents adapt via `researchIntentToBiIntent` (`lib/hrr/`) — `intent_text` = the fixed topic label, never raw reviewer text. No synthetic `UserGoal`; no HRR-local BI rule table. For HRR: `diagnostics` always `[]`; `not_met` claims are excluded from the BI feed and returned in a separate `does_not_apply[]`. |
...
| **T-2 BI generic input** — **DONE (CAH-4G.1 + CAH-4G.3)** | **`BiIntent` + `BiResult` minimal shared contracts.** `buildBoundedInterpretations(BiIntent[], BiResult[], diagnostics?, humanContributionDescription?)`. `BiResult` keeps `source_fact` nested → a CRC `RetrievalResult` structurally satisfies it → **zero CRC call-site / test churn** (the Slice-1 deferral concern is fully resolved this way). | Verified fields BI reads: intents → `{intent_id, intent_text, category, scope}`; results → `{matched_goal_category, unresolved_project_dependencies, claim_id, candidate_statement, match_origin, source_fact.kind}`. Nothing more. | `lib/bounded-interpretation/{types.ts (BiIntent + BiResult), adapters.ts (userGoalsToBiIntents), build-bounded-interpretation.ts}`; NEW `lib/hrr/bi-adapters.ts` (`researchIntentToBiIntent`, `reviewerClaimToBiResult`). CRC call sites unchanged. `tsc` clean; byte-for-byte behavior-neutral (identical 20-failing-suite / 77-failing-test set) for the `RetrievalResult`→`BiResult` swap alone. |
...
| **3 — DONE (CAH-4G.3, 2026-09-10)** | **Converged HRR research pipeline** | NEW **`lib/hrr/`** dir: `run-hrr-research.ts` (`runHrrResearch` + `topicSelectionGateResult`), `bi-adapters.ts` (`researchIntentToBiIntent`, `reviewerClaimToBiResult`), `types.ts` (`HrrResearchResult`, `HrrResearchTopicResult`). `BiResult` added to `lib/bounded-interpretation/types.ts`; `build-bounded-interpretation.ts` `RetrievalResult`→`BiResult` (3 type positions, no logic). **No route, no UI, no audit** (`runHrrResearch` is PURE — takes an already-resolved `ReviewerLkContextBundle` + the gated `HrrAuthorityGateResult`; the route wires context + audit in Slice 5/6). | `selectReviewerClaims` / `evaluateReviewerEligibility` / `evaluateApplicabilityDetailed` **byte-identical calls**; `diagnostics: []`; `not_met` → `does_not_apply[]` (excluded from BI); `humanContributionDescription: {state:'unknown'}`; no `crc_eligible`; **no synthetic `UserGoal`, no synthetic `RetrievalResult` field**; raw question never enters selection/applicability/BI (only `attributed_question`); BI stays the semantic ceiling (a `determination_request` clause → `determination_declined`, no competing HRR refusal). | **DONE:** `run-hrr-research.test.ts` (26 tests: topic shortcut ×5, free-form single, multi-topic, judgment-only, mixed, determination-request, applicability met/unresolved/not_met, reviewer-ineligible, no-coverage, Linked-CRC independence, false-premise, purity), `run-hrr-research-firewall.test.ts` (26 tests: no CRC retrieve / sessions / crc-engine / Linked-CRC / assessment write / Anthropic / UI; raw question never forwarded; only gated intent enters). `tsc` clean; `next build` exit 0; **full suite: identical 20-failing-suite / 77-failing-test set before/after** (baseline `ecba033`). | no composition prose; no `HrrResearchAnswer`; no route; no UI; no audit migration | no |
...
| ADAPTED (additive) | `buildBoundedInterpretations`: `UserGoal[]` → `BiIntent[]` (Slice 1) + `RetrievalResult[]` → `BiResult[]` (Slice 3). CRC call sites unchanged (`RetrievalResult[]` structurally satisfies `BiResult[]`). | `lib/bounded-interpretation/{types.ts, adapters.ts, build-bounded-interpretation.ts}` | **SHIPPED CAH-4G.1 + CAH-4G.3 — both proven byte-for-byte behavior-neutral for CRC.** |
| NEW type | `BiIntent`, `BiResult` | `lib/bounded-interpretation/types.ts` | **SHIPPED — `BiIntent` CAH-4G.1, `BiResult` CAH-4G.3** |
