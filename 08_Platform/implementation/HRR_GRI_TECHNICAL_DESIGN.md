...
HrrAuthorityGateResult   (free-form: classifier → hrrAuthorityGate ;  topic chip: topicSelectionGateResult())
  ...
  ─────────────────────────────────────────── Slice 6 below ──────────────────────────────────────────────
  → route + free-form UI       app/api/admin/submissions/[id]/reviewer-lk/research/route.ts + ReviewerLkLookup.tsx  (Slice 6)
```
...
3. classify intent (free-form only)                interpretResearchIntent()  — 1 model call; fail-closed → 'unsupported'
                                                    (returns NOTHING to the reviewer)
4. authority gate + resolve permitted topic(s)     hrrAuthorityGate()  /  topicSelectionGateResult()
5. resolve submission retrieval context            getSubmissionFactsForReviewerLk() → buildReviewerLkContext()

runAuditedHrrResearch()  (SHIPPED CAH-4G.5 — steps 6–9):
6. pure pipeline, IN MEMORY                         runHrrResearch() → projectHrrResearchAnswer()   (nothing returned yet)
7. sanitized audit projection                      projectHrrAuditRecord(identity, result)
                                                    → null when no governed research → skip 8, return the (refusal/paths) answer
8. PERSIST the lk_research audit event              recordReviewerLkAccess({ actorUserId, submissionId })  — THROWS on failure
      audit persistence fails → FAIL CLOSED → throw HrrAuditNotRecordedError → route answers its safe 503, ZERO governed content
9. ONLY THEN                                        return the HrrResearchAnswer
```
...
| NEW route | free-form (wraps `runAuditedHrrResearch`; the fail-closed audit-before-content boundary is already shipped in that function) | `app/api/admin/submissions/[id]/reviewer-lk/research/route.ts` | designed (Slice 6) |
...
| NEW tests | **SHIPPED:** `hrr-intent-classifier.test.ts` / `hrr-authority-gate.test.ts` / `hrr-classifier-firewall.test.ts` (`__tests__/reviewer-lk/`, CAH-4G.2); `run-hrr-research.test.ts` / `run-hrr-research-firewall.test.ts` (`__tests__/hrr/`, CAH-4G.3); **`hrr-projection.test.ts`** (`__tests__/hrr/`, CAH-4G.4 — grounding rule + 14-case matrix + wording lock + architecture firewall); **`audited-hrr-research.test.ts`** (`__tests__/hrr/`, CAH-4G.5 — audit projection purity + audit-before-content ordering + fail-closed + no-leak + no-assessment-state firewall). Designed: route presentation (Slice 6). | `__tests__/reviewer-lk/`, `__tests__/hrr/` | partial |
