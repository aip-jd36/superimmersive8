...
**Status:** `ARCHITECTURE FROZEN / IMPLEMENTATION IN PROGRESS` — **Slice 1 (BI input adaptation boundary) COMPLETE (CAH-4G.1, 2026-09-10)**; **Slice 2 (structured research-intent classifier + authority gate) COMPLETE (CAH-4G.2, 2026-09-10)**; Slices 3–7 not started. Sections A–R are the design narrative; **§S (Architecture Freeze), §T (Resolved open questions), §U (Implementation contract), §V (Frozen UAT)** are the binding implementation contract — an implementation agent follows them and makes no architecture decisions while coding. **No governed LK retrieval from free-form questions, no HRR pipeline, no HRR answer composition, no HRR route, no HRR free-form UI, no audit migration exists yet. CRC behavior is unchanged.**
...
5. **Zero CRC change.** `lib/crc-engine/**`, `lib/interview-engine/**` (except the additive `BiIntent` generalization of `buildBoundedInterpretations` — §S-5/§U slice 1, **SHIPPED CAH-4G.1 with a proven byte-for-byte CRC regression**; `BiResult` deferred), CRC routes, CRC UI: untouched.
...
**Current (post-CAH-4G.1 Slice 1, 2026-09-10):** `buildBoundedInterpretations(intents: BiIntent[], results: RetrievalResult[], diagnostics?, humanContributionDescription?)` (`build-bounded-interpretation.ts`). It reads only `intent.{intent_id, intent_text, category, scope}` and, per result, `{matched_goal_category, unresolved_project_dependencies, claim_id, candidate_statement, match_origin, source_fact.kind}`.
...
- A research-intent caller builds `BiIntent` directly from `ExplicitResearchIntent` (`intent_id` = a per-request id; `intent_text` = for a topic pick, the fixed topic label; for a question, the attributed quotation — **used only as `BoundedInterpretation.goal_text`, which BI renders verbatim and never transforms**, per the PM revision-6 note; `category` = `intent.topic`; `scope` = `intent.scope`). **No synthetic `UserGoal`.**
- Research provenance (`interpreted`, `source_kind`, `source_text_ref`) is carried alongside by the research caller, keyed by `intent_id` — it never enters `BiIntent` or BI.

**`ExplicitResearchIntent → BiIntent` adapter: DEFERRED to Slice 3 (CAH-4G.2 decision, 2026-09-10).** Slice 2 produces `ExplicitResearchIntent[]` but nothing runs BI yet — there is no Slice-2 consumer. It also cannot live in `lib/reviewer-lk/**`: the authority firewall (`__tests__/reviewer-lk/authority-firewall.test.ts`) forbids any `bounded-interpretation` import there, and a `BiIntent`-typed import would trip it for zero runtime benefit. The adapter (`researchIntentToBiIntent`) lands with `runHrrResearch()` in the converged-pipeline slice — outside the firewall boundary, next to `reviewerClaimToBiResult` and the `buildBoundedInterpretations` call. This mirrors the Slice-1 `BiResult` deferral: the adapter ships with its first real consumer, not speculatively.

**Second adaptation boundary — results (`BiResult`): DEFERRED to the HRR-runtime slice (CAH-4G.1 refinement, 2026-09-10).** `buildBoundedInterpretations` reads a `RetrievalResult[]` and (verified at `238670c`) touches per result: `matched_goal_category` (the §198 filter), `unresolved_project_dependencies`, `claim_id`, `candidate_statement`, `match_origin` (`=== 'related_topic'`), `source_fact.kind` (`=== 'tool'`). `RetrievalResult` is already the established generic cross-subsystem contract (`lib/retrieval-engine/types.ts`; Projection consumes it the same way), so **Slice 1 keeps `results: RetrievalResult[]` unchanged** — there is no fabrication risk on the results side and no Slice-1 consumer for a narrower type. HRR's selector emits `ReviewerLkClaim[]`, not `RetrievalResult[]`, so a pure adapter **`reviewerClaimToBiResult(claim, topic)`** is still required — it ships **with the HRR research runtime** (the converged-pipeline slice), next to its first real consumer, mapping: `matched_goal_category = topic`; `candidate_statement = claim.statement`; `claim_id`, `unresolved_project_dependencies` verbatim; `match_origin = 'exact_topic'`; `source_fact = { kind: 'topic' }`. Whether its target is a 6-field `BiResult` subset or a constructed `RetrievalResult` is that slice's call — either keeps BI's contract small; deciding it now (with no consumer) would only be speculative machinery.
...
| S-5 | **BI adaptation** (§H FROZEN; input side SHIPPED CAH-4G.1 Slice 1, result side refined 2026-09-10): `buildBoundedInterpretations` takes `BiIntent[]` (`{intent_id, intent_text, category, scope}`). CRC adapts via `userGoalsToBiIntents` (1-line call-site change, **zero behavior change, proven**). BI no longer applies a lifecycle filter itself. **Results stay `RetrievalResult[]`** — already the generic cross-subsystem contract; a narrower `BiResult` had no Slice-1 consumer and is **deferred to the HRR-runtime slice**, where `reviewerClaimToBiResult` (mapping a `ReviewerLkClaim` → BI's result contract) lands with its first real consumer. No synthetic `UserGoal`; no HRR-local BI rule table. For HRR: `diagnostics` always `[]`; `not_met` claims are excluded from BI matches and shown in a separate "does not apply" block. |
...
| **T-2 BI generic input** | **`BiIntent` minimal shared contract — SHIPPED (CAH-4G.1 Slice 1).** `buildBoundedInterpretations(BiIntent[], RetrievalResult[], diagnostics?, humanContributionDescription?)`. Results stay `RetrievalResult[]` (already generic); the narrower `BiResult` + `reviewerClaimToBiResult` are **deferred to the HRR-runtime slice** (no Slice-1 consumer — deciding the exact result shape now would be speculative). | Verified fields BI reads: intents → `{intent_id, intent_text, category, scope}` (adapter applies the `superseded_by === null && state === 'confirmed'` filter that used to live in BI); results → `{matched_goal_category, unresolved_project_dependencies, claim_id, candidate_statement, match_origin, source_fact.kind}`. Nothing more. | `lib/bounded-interpretation/{types.ts (BiIntent), adapters.ts (userGoalsToBiIntents), build-bounded-interpretation.ts (signature)}`; CRC + canary + script call sites wrapped; test suites route through the adapter via a per-file shim (zero-behavior-change proof). `reviewerClaimToBiResult` ships with the HRR runtime slice. |
...
| **3 (next)** | **Converged HRR research pipeline** | NEW `lib/reviewer-lk/run-hrr-research.ts` (`ExplicitResearchIntent[] → selectReviewerClaims → reviewerClaimToBiResult → buildBoundedInterpretations`), the **`researchIntentToBiIntent`** adapter (`ExplicitResearchIntent → BiIntent`, deferred from Slice 2 — lands here, outside the `lib/reviewer-lk/**` authority-firewall boundary) + the **`reviewerClaimToBiResult`** adapter (deferred from Slice 1), refactor `app/api/admin/submissions/[id]/reviewer-lk/route.ts` topic path to call it + NEW `…/research/route.ts` for free-form | `selectReviewerClaims` / `evaluateReviewerEligibility` / `evaluateApplicabilityDetailed` **byte-identical calls**; `diagnostics: []`; `not_met` excluded from matches; no `crc_eligible`; **no synthetic `UserGoal`** | `run-hrr-research.test.ts` (single/multi-topic, `outside_current_coverage`, `not_met` separation, per-clause scope → `determination_declined` for a `determination_request` clause); reuse `reviewer-lk/authority-firewall.test.ts` for both routes; `select-reviewer-claims` unchanged (its tests still pass) | no composition prose yet (return the structured `HrrResearchAnswer` skeleton); no UI | no |
...
| ADAPTED (additive) | `buildBoundedInterpretations` input: `UserGoal[]` → `BiIntent[]` + `userGoalsToBiIntents` adapter (CRC call site: 1-line change, 0 behavior change) | `lib/bounded-interpretation/{types.ts, adapters.ts, build-bounded-interpretation.ts}` | **SHIPPED CAH-4G.1 Slice 1 — byte-for-byte CRC regression proven. Results stay `RetrievalResult[]`; `BiResult` deferred to the HRR-runtime slice.** |
| NEW type | `BiIntent` | `lib/bounded-interpretation/types.ts` | **SHIPPED CAH-4G.1 Slice 1** |
...
| NEW type | `HrrResearchAnswer` | `lib/reviewer-lk/types.ts` | designed (Slice 4) |
...
| NEW module | `projectHrrResearchAnswer()` (deterministic composition) | `lib/reviewer-lk/project-hrr-research-answer.ts` | designed (Slice 4) |
| NEW route | free-form (audited; audit-before-content) | `app/api/admin/submissions/[id]/reviewer-lk/research/route.ts` | designed (Slice 3/6) |
...
| NEW tests | **SHIPPED CAH-4G.2:** `hrr-intent-classifier.test.ts`, `hrr-authority-gate.test.ts`, `hrr-classifier-firewall.test.ts` (all source-scan / pure / mocked-SDK, no render harness, no live Anthropic). Designed: `run-hrr-research.test.ts` (Slice 3), `hrr-projection.test.ts` grounding rule (Slice 4), route firewall/audit (Slice 5), presentation (Slice 6). | `__tests__/reviewer-lk/` | partial |
