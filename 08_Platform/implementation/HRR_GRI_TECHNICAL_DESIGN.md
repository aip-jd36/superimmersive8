...
**Status:** `IMPLEMENTED / REBASED CLEAN ONTO main / PREVIEW PUSHED / CREDENTIALED VISUAL UAT + LIVE CLASSIFIER EVAL PENDING` — **Slices 1–7 COMPLETE** (CAH-4G.1…CAH-4G.7, 2026-09-10). **CAH-4G.7P (2026-09-10):** the full 19-commit CAH-4G series was rebased onto `origin/main` (`eb4641f`) — `git range-diff` shows all 19 commits patch-identical (`=`), no conflicts (zero file overlap with the Firefly/Veo drift); post-rebase `tsc` clean, `next build` exit 0, full-suite failing set **byte-identical to a fresh `eb4641f` baseline** (77 failing on both — the pre-existing `runway-gen3` cascade — zero new failures; +295 new passing / +2 CRC suites run identically). The rebased HEAD (`e6ddc35`) was pushed to **`origin/work/cah-4g-current-main-integration`** (new branch, no force; `origin/main` untouched) to enable a Vercel Preview. **The authenticated browser visual UAT (`REVIEWER_RESOURCES_ARCHITECTURE.md §18`) and the bounded live classifier cost/quality eval (§V-7) were NOT performed — this build environment has no Vercel auth, no admin/reviewer credentials, no `ANTHROPIC_API_KEY`, and no access to a synthetic-submission DB. Both remain hard gates before any merge to `main` or production deployment.** Sections A–R are the design narrative; **§S/§T/§U/§V** are the binding implementation contract. **The Living Knowledge tab is the converged Governed Research Interface: topic shortcuts + Ask HRR both POST `/api/admin/submissions/[id]/reviewer-lk/research` → `runAuditedHrrResearch()` (audit-before-content, fail-closed) → one `HrrResearchAnswer` → one `<HrrResearchAnswerView>`. Single-turn; raw question never persisted. The legacy `GET .../reviewer-lk` topic route is RETIRED (CAH-4G.7 — its fail-closed audit contract transferred to `hrr-research-route.test.ts`). CRC unchanged.** **Not `CLOSED` / `PRODUCTION-PROVEN` — production deployment and an authenticated visual UAT have not happened. Slice 7 performed a server-rendered-markup composition-quality review (real components + real pipeline data, not an auth browser session — no safe preview/local path exists in the build environment), applied renderer-only consultative hardening (CAH-4G.7 `fix` commit), retired the dead legacy route, kept `HRR_MAX_RESOLVED_TOPICS = 2`, and left the live classifier cost/quality eval as a preview/production UAT item.**
