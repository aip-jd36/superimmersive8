...
**Status: PM DECISIONS RECORDED (2026-08-17).** Architecture/design only — still no governed claim, relationship, Matrix row, `GoalCategory`, `ApplicabilityFact`, database schema, migration, or production code is created or modified by this document at any point in its history, including this update. The sections below now distinguish **APPROVED (direction only, not implementation)**, **DEFERRED (future/backlog)**, and **NOT APPROVED** per PM's Phase 0.5 closure review — see the PM Decision Record immediately below. Where PM's decision changed a prior recommendation in this document, the original reasoning is left intact and the resolution is noted inline, so the document remains an honest record of how the design was pressure-tested, not a silent rewrite.

## PM Decision Record (2026-08-17 closure)

| Area | Decision |
|---|---|
| `third_party_source_rights` GoalCategory | **APPROVED as architectural direction.** Path A (explicit question → goal extraction → ordinary Topic Retrieval → governed `TopicClaim`s → Bounded Interpretation → Projection) is the correct future shape. **Not implemented** — see "Implementation gating" below. |
| Separate provider/source entity | **APPROVED as architectural direction.** Final chosen name: **`AssetProviderMention`** — see §7. |
| List semantics (multiple providers) | **APPROVED** — a list field, never a singular value. See §10. |
| `canonical` / `unresolved_alias` resolution pattern | **APPROVED** — reused as a pattern, not by reusing `ToolMention` itself. See §7/§10. |
| Matrix | **APPROVED unchanged** — no expansion, Getty/iStock/Adobe Stock/Shutterstock never become Matrix rows. |
| `ProjectFacts` | **APPROVED unchanged** — zero new fields. See §9. |
| `ApplicabilityFacts` | **APPROVED unchanged** — zero new values. See §8. |
| `TopicClaim` | **APPROVED unchanged** — remains the sufficient governed-object type. See §17. |
| `ScopedObservation` | **APPROVED unchanged** — retains its existing catch-all role, no stock-specific semantics. See §9. |
| `unresolved_project_dependencies` | **APPROVED reused unchanged** — informational only, never a pseudo-fact. See §9. |
| Path A engineering implementation | **NOT YET AUTHORIZED.** Direction is approved; building it is gated on a real governed claim demonstrating the need — knowledge first, product behavior second. See "Implementation gating" below. |
| Path B (discovered relevance from incidental disclosure) | **DEFERRED — future architecture/backlog, not solved in this milestone.** See §5 (unchanged reasoning) and the new note there. |
| `commercial_use → third_party_source_rights` TopicRelationship | **NOT APPROVED.** This document's earlier recommendation (§13, original text preserved below with a correction notice) is **superseded by PM decision**: the copyright case (`copyright_ownership → copyrightability`) is a relationship between two *legal topics* that are substantively relevant to each other; this case is relevance conditioned on a *project condition* (whether third-party source material was used at all), which a goal-to-goal relationship cannot honestly represent without becoming exactly the "generic routing glue" `TOPIC-RELATIONSHIPS.md` itself warns against. No `REL-STOCK-*` record, Candidate or otherwise, is created. Mechanism remains genuinely unresolved, pending Path B / Project-Fact-Aware Bounded Composition design. |
| Generic applicability-trigger workaround | **NOT APPROVED** — confirms this document's own §6/§8 finding; recorded as a decision, not just an engineering conclusion. |
| Getty-specific routing | **NOT APPROVED** — no provider-specific mechanism; `AssetProviderMention`'s alias registry may contain Getty as one entry among several, exactly as `KNOWN_TOOLS` contains many tools, never a Getty-only code path. |
| First governed-claim direction (Editorial-use restriction) | Recorded as **recommended next research target only** — not adopted, not drafted as `CLAIM-STOCK-*`, not marked Under Review. See §17 note. |

**Implementation gating (PM instruction, 2026-08-17):** Path A's architecture is approved *as a direction*, not as authorization to build. PM's stated ordering is **knowledge first, product behavior second** — no governed stock-media `TopicClaim` currently exists (Phase 0's `CAND-STOCK-*` entries remain Candidate-only, un-adopted), so there is nothing yet for Path A's infrastructure to serve. The next milestone is substantive research/candidate-claim definition (see §17), not engineering.
...
- **(C) New sibling entity type — recommended.** Smallest correct fit: reuses `ToolMention`'s already-proven `resolution: {kind:'canonical', identifier} | {kind:'unresolved_alias', raw_name}` shape verbatim (same alias-registry pattern, same "never silently canonicalize an ambiguous name" discipline, same normalize→attest→mutate pipeline shape) but as its **own** type and its **own** `StructuredUnderstanding` field, sibling to `tool_mentions`, never merged into it. No `access_surface`/`plan_tier` fields — this entity answers "which provider," nothing else. (Naming resolved separately in §7a below, per PM's 2026-08-17 challenge to the original placeholder name.)
...
**Recommended: (C).** Getty is confirmed (Phase 0 §1, §17) to be a structurally different *kind* of thing than Kling — a source/licensing provider, not a generation platform — and the architecture should say so directly rather than encode the distinction as an implicit convention. This is the smallest representation that (a) fixes the literal "wasn't able to match to a specific platform yet" mis-signal (a recognized instance of this new type is not a failed match — it's a correctly-recognized different kind of entity; future rendering, not written here, could say so), (b) supports Path A cleanly, and (c) is exactly the data Path B's eventual capability (§5) would need as an input, without pre-building that capability now. **APPROVED as architectural direction (PM, 2026-08-17)** — see PM Decision Record.

### 7a. Naming resolution (PM challenge, 2026-08-17)

The original draft of this document used `SourceAssetMention` as a placeholder. **PM correctly challenged this as semantically wrong: Getty is not itself a source asset — it is the provider/source *of* assets.** A record named `SourceAssetMention` reads as "a mention of a source asset," which misdescribes what is actually being captured. Re-pressure-tested against six candidates, evaluated against the four platforms researched (Getty, iStock, Adobe Stock, Shutterstock) and the five plausible future categories named in the task (music libraries, stock-footage providers, font providers, template marketplaces, 3D-asset providers), and against four specific false-assumption traps:

| Option | Reads naturally for Getty/iStock/Adobe Stock/Shutterstock? | Reads naturally for future categories (music/footage/fonts/templates/3D)? | Assumes licensing? | Assumes "stock media" specifically? | Assumes commercial use? | Assumes the asset appears in final output? |
|---|---|---|---|---|---|---|
| A. `SourceAssetMention` (original) | No — names the asset, not the provider; the actual semantic bug PM flagged | No, same bug | No | No | No | No |
| B. `SourceProviderMention` | Yes, but "source" is redundant next to "provider" (a provider already implies being a source) | Yes, slightly awkward ("a font foundry is a source provider" reads oddly) | No | No | No | No |
| C. `AssetProviderMention` | Yes, cleanly — "Getty is an asset provider" | Yes, cleanly — "Adobe Stock/a font foundry/a 3D-asset marketplace is an asset provider" for every listed future category | No | No | No | No |
| D. `ThirdPartySourceMention` | Ambiguous — doesn't say whether it's the provider or the material itself being mentioned | Same ambiguity | No | No | No | No |
| E. `LicensedAssetProviderMention` | Reads well for a validly-licensed case, but **bakes in an assumption the entity must NOT make** (task's own explicit trap: "every provider is licensed") — fails a stress test directly, e.g. a client-supplied asset of unknown licensing status, or a free/unlicensed download | Same flaw, worse at scale (music/fonts/templates have equally common free/unlicensed variants) | **Yes — rejected on this basis alone** | No | No | No |
| F. Other considered and rejected: `StockMediaMention` (fails immediately — assumes "stock media" specifically, excluding client-supplied brand assets already in scope per Phase 0 §14) | — | — | No | **Yes — rejected** | No | No |

**Recommended and final: `AssetProviderMention`.** Reads correctly today and for every named future category, makes no licensing/commercial/stock-media/appears-in-output assumption, and follows the existing `<Role>Mention` naming pattern (`ToolMention` = tool + Mention) exactly. `SourceProviderMention` was the closest runner-up and is recorded here in case a future engineer wants the reasoning for why it wasn't chosen (marginal — "asset" is the more precise noun for what's actually being provided; "source" doubles up with "provider").

**Exact semantics of one `AssetProviderMention` record (PM instruction, 2026-08-17 — must be stated explicitly, not left implicit):**

> The user named an external provider/source of third-party material potentially associated with the project. This records recognition of the **named provider only**.

It does **not** mean, and must never be read to imply:
- that SI8 verified any license;
- that the named asset(s) were validly licensed;
- that the named asset(s) are commercially usable;
- that the named asset(s) appear in the final output;
- that the provider granted AI-input/upload rights;
- that the user owns the asset;
- that the provider is an AI-generation tool (that is `ToolMention`'s role, a structurally distinct type — see §7 A/B above);
- **that any specific asset from this provider was confirmed actually used in the project** (added beyond the task's own list, because §17's stress test proved even *asset-level* facts from a single confirmed provider can't be safely flattened into one record — the provider-level record is weaker still, and must not imply asset-level confirmation it cannot carry).

### 7b. Entity shape confirmation

PM approved reuse of the **pattern**, not reuse of `ToolMention` the type. Confirmed as the smallest appropriate v0.1 shape — no asset inventory, no license/classification/purchaser/account fields (all explicitly out per PM instruction; all remain either `ScopedObservation` free text or Commercial Assurance evidence, per §9/§12/§18 below):

```
AssetProviderMention {
  mention_id: string
  resolution: { kind: 'canonical'; identifier: string }
...
No `access_surface`, no `plan_tier` (both `ToolMention`-specific, both meaningless for a provider), no asset ID, no license ID, no content classification, no purchaser/account-owner field, no per-asset records, no license-validity field. This is illustrative shape for the design record — not a type declaration to be copy-pasted at implementation time without its own review.
...
|---|---|
| **Client-supplied** ("My client gave me a Getty image") | `AssetProviderMention{resolution: canonical, identifier: 'getty'}` recorded. Nothing else inferred — `workflow_role`/relationship-to-client stays exactly what `ProjectFacts.workflow_role` and free-text `ScopedObservation` already capture today, unchanged. No claim about license validity is fabricated. Matches task's own requirement: represent the state without pretending unknowns are known. |
| **User-licensed + uploaded to Kling** | Two independent, correctly-typed mentions coexist: `ToolMention{identifier:'kling'}` (existing, unchanged) and `AssetProviderMention{identifier:'getty'}` (new). Neither overwrites or nests inside the other — this is exactly why (A)/(B) in §7 were rejected. Manner of use (Creative classification, "uploaded into," company-account relationship) remains observation/free-text in v0.1, per §9. |
| **Reference-only, not in final output** | `AssetProviderMention` recorded; the "reference only" qualifier stays in `ScopedObservation.note` (unstructured). The architecture does **not** attempt to resolve whether this satisfies or evades Getty's own AI-input restriction — matches the task's explicit instruction not to force a conclusion; Phase 0 §24 already flagged this exact scenario as genuinely unresolved even at the legal-research level. |
| **Editorial case** | Utterance → `AssetProviderMention{identifier:'getty'}` (Path B, no goal) or, if phrased as a question, a `third_party_source_rights` goal (Path A) → under Path A only, topic retrieval candidates a hypothetical `CAND-STOCK-003`-descended claim → (no applicability gate, per §8) → Bounded Interpretation renders `directly_relevant` with the claim's own governed text, exactly like any existing topic claim. Under Path B, nothing retrieves — disclosed limitation (§5). |
...
| **Multiple providers** ("Getty, Adobe Stock, and Kling") | `asset_provider_mentions: [AssetProviderMention(getty), AssetProviderMention(adobe-stock)]` + `tool_mentions: [ToolMention(kling)]` — a list, exactly like `tool_mentions` already is a list for multiple AI tools in one project. No new list-handling logic required; the type already supports plurality by being an array. |
| **Multiple assets, same provider, different classification** ("two Creative, one Editorial from Getty") | **This is the clearest evidence for where CRC's abstraction boundary should stop.** A single `AssetProviderMention` per provider (not per asset) is the v0.1 recommendation — CRC does not build an asset inventory. Per-asset classification is real, necessary information, but it is **evidence-level**, squarely inside Commercial Assurance's reviewer-inspection role (Phase 0 §8's evidence table), not a CRC conversational-capture concern. If a user volunteers "two were Creative, one was Editorial," that nuance stays in free-text `ScopedObservation`, available for a human reviewer to read verbatim, never flattened into a misleading single structured value. |
...
| 3 — Broad goal + observed entity (new join-gate: `commercial_use` AND `AssetProviderMention` exists) | Low, if built correctly | Low | Medium | **Requires new retrieval mechanism** — exactly PRD §27's deferred capability | **No — does not exist today; would be new Retrieval logic** | **High**, and duplicative of already-deferred work |
...
## 13. TopicRelationship decision — SUPERSEDED BY PM DECISION (2026-08-17)

**PM correction, recorded 2026-08-17 — this section's original recommendation (preserved below, struck by decision not by rewrite) is NOT APPROVED.** Original reasoning, kept for the record:

> Task §20 asks directly: is this genuinely a `TopicRelationship`, or just applicable knowledge once a fact exists? Given §6/§8's finding that a fact-conditioned applicability model doesn't safely exist yet, a `TopicRelationship` is not being used as "generic routing glue" here — it is being used for exactly what it already means: `commercial_use → third_party_source_rights` would state that knowledge under the target topic *may be relevant* to a `commercial_use` goal, without answering it — precisely `REL-COPY-OWNERSHIP-COPYRIGHTABILITY-v1`'s own shape and its own already-accepted caveat (a relationship can be `Adopted` while `CRC Eligible: Pending` indefinitely). Recommended posture, explicit: draft the relationship's governance record (Candidate, later possibly Adopted) whenever real claims exist under `third_party_source_rights`, but keep `CRC Eligible: Pending` until either (a) every claim reachable through it has `applicability_requirements: []`, or (b) Path B's future composition capability changes what "activating a relationship" even means.

**PM's correction:** the analogy to `REL-COPY-OWNERSHIP-COPYRIGHTABILITY-v1` does not hold as cleanly as the original text claimed. `copyright_ownership → copyrightability` connects two *legal topics* that are substantively relevant to each other regardless of project specifics — any copyright-ownership question benefits from copyrightability knowledge, structurally, every time. `commercial_use → third_party_source_rights` is different in kind: third-party-source knowledge is relevant *only when a project condition holds* (third-party source material was actually used), not because `commercial_use` and third-party-source-rights are substantively linked legal topics on their own terms. Drafting the relationship anyway — even Candidate-only, even kept `CRC Eligible: Pending` — would misuse `TopicRelationship`'s own defined meaning (`TOPIC-RELATIONSHIPS.md`: "may knowledge under one topic be relevant to interpreting a user's goal under a different topic, without that knowledge itself answering the goal") to paper over a condition-gated relevance question the mechanism was never built to express. This is exactly the "generic routing glue" `TOPIC-RELATIONSHIPS.md` already warns against, even though the original text above argued (incorrectly) that it wasn't.

**Decision: no `REL-STOCK-*` record — Candidate or otherwise — is created. No `TopicRelationship` fixture is touched.** Recorded only as: **potential future routing/relationship requirement; exact mechanism unresolved, pending Path B / Project-Fact-Aware Bounded Composition design** (§5, §PM Decision Record above).
...
- `AssetProviderMention` — the same entity shape (`resolution: canonical | unresolved_alias`) fits a music-licensing service, a font foundry, or a client's own brand-asset handoff equally well; only the alias registry's contents would need to grow, not the type.
...
| B | Entity representation | **Add one new type, `AssetProviderMention`** (name resolved and final — PM decision, 2026-08-17, see §7a), structurally parallel to `ToolMention` (same `resolution` shape, same alias-registry pattern) but a distinct type with no `access_surface`/`plan_tier` fields. New `StructuredUnderstanding.asset_provider_mentions: AssetProviderMention[]` field, sibling to `tool_mentions`. |
...
| F | `TopicRelationship` | **NOT APPROVED (PM decision, 2026-08-17 — see §13).** No `REL-STOCK-*` record, Candidate or otherwise. Mechanism for surfacing this domain from `commercial_use` remains unresolved, pending Path B / Project-Fact-Aware Bounded Composition design. |
...
- "I sourced the other images myself from Getty Images": under this design, extraction proposes an **`asset_provider_mention` candidate** (new `CandidateObservation.kind`, mirroring `tool_mention`'s own shape) → normalizes against the new Getty/iStock/Adobe Stock/Shutterstock alias registry → resolves to `AssetProviderMention{resolution: {kind:'canonical', identifier:'getty'}, source_turn: 2, source_statement: "I sourced the other images myself from Getty Images."}` — **no longer falls into `unresolved_aliases`**, because it is no longer being incorrectly evaluated against the AI-tool registry at all.
...
| `ToolMention`'s resolution shape | Reused as a pattern (not the type itself) for `AssetProviderMention` (§7). |
...
| Surface | Touched? | Why |
|---|---|---|
| Schema/type changes | Yes — small | One `GoalCategory` enum value; one new interface (`AssetProviderMention`); one new `StructuredUnderstanding` field. |
| Extraction | Yes — small–medium | New `CandidateObservation.kind: 'asset_provider_mention'` branch (mirrors `tool_mention`'s existing shape closely); new alias registry; extractor prompt update to recognize asset-provider mentions and, separately, explicit third-party-rights questions (`goal_category_hint`). **Not authorized for implementation — direction only (see PM Decision Record).** |
...
| Projection/email | **No change** — `understood-summary.ts` would need one new rendering branch for `asset_provider_mentions` (distinct from `unresolved_tool_mentions`) to stop the misleading "wasn't able to match to a specific platform yet" sentence for a recognized provider — small, but real; not specified further here (no CRC copy authored). |
| Analytics | **No change.** |
...
## 23. Closure — next milestone (PM decision, 2026-08-17)

Per the PM Decision Record at the top of this document: the architectural direction in §18 is approved (with §13/relationship reversed to NOT APPROVED, per that section's correction), but **no implementation begins from this document.** PM's explicit ordering: **knowledge first, product behavior second** — there is no Adopted, CRC-eligible stock-media `TopicClaim` yet for Path A's infrastructure to serve, so building it now would be infrastructure without a payload.

**Recommended next milestone: substantive candidate-claim research**, not engineering. Phase 0's Editorial-use-restriction finding (`CAND-STOCK-003` in the Phase 0 research artifact — narrow, cross-platform-consistent, highest confidence of the five candidates) is recorded here only as a **recommended next research target**, not adopted, not drafted as `CLAIM-STOCK-*`, not marked Under Review, and not a CRC-publication decision. That work is explicitly out of scope for this document.

---

**Design conducted:** 2026-08-17. **PM decisions recorded:** 2026-08-17 (this closure pass). **Status:** Architectural direction approved (see PM Decision Record); implementation NOT authorized; relationship NOT approved; Path B deferred. **Next step:** substantive stock-media candidate-claim research (separate future milestone) — no implementation should proceed from this document without a further, separate PM decision once that research produces governed knowledge that actually needs Path A's infrastructure.
