...
**Status:** `CAH-4G.11 — FOLLOW-UP DISCOVERY / ARCHITECTURE VALIDATION — DESIGN / EVIDENCE COLLECTION` (2026-09-11). **Superseded in scope by `HRR_RESEARCH_SESSION_CONTRACT.md` (CAH-4G.12) and executed as `HRR_SESSION_DISCOVERY.md` (CAH-4G.13)** — this document's taxonomy/classification framework remain valid and are reused there under the narrower topic-scoped-session frame; its own broader 17-question script is not the one PM executes. No runtime change. `CAH-4G.10 SLICE A remains CLOSED / PRODUCTION-PROVEN` (unchanged, not reopened by this document). **Slice B remains NOT STARTED / NOT AUTHORIZED.** The Slice-B design sketched in `HRR_CONVERSATIONAL_ARCHITECTURE.md §J` (`HrrThreadContext`) was a **design hypothesis**, never implemented, never PM-authorized — this document does not promote it to a requirement; it re-derives the actual evidence needed to decide, from first principles, using the now-production-proven visible thread as the discovery instrument. This document is the deliverable of CAH-4G.11: a taxonomy, a pre-registered discovery experiment, a failure classification, and an evidence threshold — **not code, not a Slice-B commitment.**
