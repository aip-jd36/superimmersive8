...
## 15. CAH-4G — HRR converged interface (as-built, CAH-4G.6)
...
**Status (CAH-4G.9, 2026-09-10):** PM production UAT PASSED the single-turn HRR experience (initial surface, topic shortcut, `copyright_ownership`, unresolved `copyrightability`, free-form classification, authority boundaries) but found the **single-turn model too visible** — a second free-form question replaces the first; reviewers expect a **conversational research thread**. Not an unsafe defect / not a rollback. Conversational-research architecture designed (docs only, no code) in **`HRR_CONVERSATIONAL_ARCHITECTURE.md`** + **`lib/reviewer-lk/ADR-003-hrr-conversation-context.md`**: append-only client-only thread; bounded enum-only follow-up referent (never transcript-to-LLM); one-model-call ceiling; no persistence / audit / schema / CRC change. This §18 script stays the pre-`CLOSED` production-UAT gate; a conversational-thread UAT addendum will be added when Slices A→B land. **Status (CAH-4G.8):** CAH-4G is **merged to `main` (`0413291`, fast-forward, no force) and production-deployed to `app.superimmersive8.com`** under a PM-authorized controlled production integration for internal synthetic UAT (route topology confirms the deploy: `POST .../reviewer-lk/research` → `401 {"error":"Unauthorized"}`, legacy `GET .../reviewer-lk` → `404`). **This script has still NOT been run in an authenticated browser** — the Claude Chrome extension was not connected to the CAH-4G.8 session and no admin/reviewer credential, `ANTHROPIC_API_KEY`, or audit-DB access was available there. Slice 7's server-rendered-markup review of the 7 answer cases remains the best composition evidence (structure/text/density verified; Tailwind styling not visually confirmed). **This script is now the post-deploy production-UAT gate — run it against `app.superimmersive8.com` before CAH-4G is marked `CLOSED / PRODUCTION-PROVEN` or used for any customer purpose.** Note: `likeness` is no longer an empty-coverage topic (NY §396-b claim on `main` since `9219496`); for a non-New-York fixture it renders applicability-unmet, not "outside current coverage".
