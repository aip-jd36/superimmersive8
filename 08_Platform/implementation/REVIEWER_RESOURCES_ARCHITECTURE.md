...
**Status (CAH-4G.7):** this script has NOT been run in an authenticated browser — the CAH-4G build slices had no deployed/preview environment with credentials. Slice 7 instead server-rendered the 7 answer cases from the real components + real pipeline data (structure/text/density verified; Tailwind styling not visually confirmed). **This script is the pre-deploy gate — run it before any production deployment.**
