...
**Status (CAH-4G.10C, 2026-09-11) — §18 conversational-thread addendum EXECUTED, PASSED:** a PM-performed authenticated production UAT against `CA-RLK-2a PROD SMOKE` / `ASSESS-007-2026-09-07` confirmed all of: heading "Research conversation"; action "Clear conversation"; a topic pick + two successive free-form questions all remained visible together (append-only, not replace); the composer cleared after each submission; Living Knowledge ↔ Linked CRC Context tab-switch preserved the thread; Reviewer Resources close/reopen preserved the thread; leaving and re-entering the workbook cleared the thread (the V1 client-memory contract, not a defect); "Clear conversation" reset the thread with no assessment-state change. Full evidence: `HRR_CONVERSATIONAL_ARCHITECTURE.md §FF`. **CAH-4G.10 Slice A — CLOSED / PRODUCTION-PROVEN** (the VISIBLE thread only). Non-blocking observations recorded, not fixed: thread density in the narrow rail on long answers; the reviewer's question rendering twice (once as the visible turn, once inside `HrrResearchAnswerView`'s own echo); whether HRR research should survive ordinary workbook navigation is an open **future** product question, not a Slice-A gap (`§HH`). Bounded conversational context (Slice B / referent resolution) is a **separate, not-started, not-authorized** milestone — each question remains fully independently researched. **CAH-4G.11 (2026-09-11):** `HRR_FOLLOWUP_DISCOVERY.md` defines a generic follow-up taxonomy, a pre-registered discovery question set, and the evidence threshold PM production use must clear before Slice B could ever be authorized — design/evidence-collection only, no runtime change. **CAH-4G.12 (2026-09-11):** PM has since made a further product-boundary decision — HRR supports a bounded, topic-anchored **Research Session** focused on one issue at a time, never an unlimited conversational assistant over the whole workbook. `HRR_RESEARCH_SESSION_CONTRACT.md` is now the one authoritative document for that contract (Research Session/Focus definition, bounded-context candidate fields, authority-firewall-inside-a-session analysis, workbook handoff, and a revised, narrower production discovery plan) — design only, no runtime change, bounded session context NOT IMPLEMENTED, Workbook Research Log FUTURE / NOT AUTHORIZED. **Prior status (CAH-4G.10I, 2026-09-11):** CAH-4G.10P proved a **DEPLOYMENT GAP** (Slice A reviewed locally, never on `main`); resolved by rebasing + fast-forward-pushing — `origin/main` == `4b9ee5a`. **Status (CAH-4G.9, 2026-09-10):** PM production UAT PASSED the single-turn HRR experience but found the **single-turn model too visible** — reviewers expect a **conversational research thread**. Not an unsafe defect / not a rollback. Architecture designed in **`HRR_CONVERSATIONAL_ARCHITECTURE.md`** + **`lib/reviewer-lk/ADR-003-hrr-conversation-context.md`**: append-only client-only thread; bounded enum-only follow-up referent (never transcript-to-LLM); one-model-call ceiling; no persistence / audit / schema / CRC change. **Status (CAH-4G.8):** CAH-4G is **merged to `main` (`0413291`, fast-forward, no force) and production-deployed to `app.superimmersive8.com`** under a PM-authorized controlled production integration for internal synthetic UAT (route topology confirms the deploy: `POST .../reviewer-lk/research` → `401 {"error":"Unauthorized"}`, legacy `GET .../reviewer-lk` → `404`). **This script has still NOT been run in an authenticated browser** — the Claude Chrome extension was not connected to the CAH-4G.8 session and no admin/reviewer credential, `ANTHROPIC_API_KEY`, or audit-DB access was available there. Slice 7's server-rendered-markup review of the 7 answer cases remains the best composition evidence (structure/text/density verified; Tailwind styling not visually confirmed). **This script is now the post-deploy production-UAT gate — run it against `app.superimmersive8.com` before CAH-4G is marked `CLOSED / PRODUCTION-PROVEN` or used for any customer purpose.** Note: `likeness` is no longer an empty-coverage topic (NY §396-b claim on `main` since `9219496`); for a non-New-York fixture it renders applicability-unmet, not "outside current coverage".
