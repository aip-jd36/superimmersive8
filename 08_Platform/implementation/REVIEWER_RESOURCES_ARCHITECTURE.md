...
**Legacy.** `GET /api/admin/submissions/[id]/reviewer-lk?topic=` (CAH-4E) is **retained but has no UI consumer** — its retirement (with `route-and-audit.test.ts`, firewall refs, `projectReviewerLkResult`, `ReviewerLkLookupResult`) is a Slice-7 cleanup.
...
## 18. CAH-4G.6 UAT script (manual — no browser harness in this repo)

**Environment:** a deploy of the CAH-4G.6 branch. Sign in as an admin (`users.is_admin = true`). Open `/admin/submissions/{id}/review` for an internal synthetic Commercial Assurance fixture (e.g. `ASSESS-007-2026-09-07`). `ANTHROPIC_API_KEY` must be set for the Ask-HRR steps (topic steps do not need it).
...
| 3 | Expand *Governed claims — applicability & provenance* and *Provenance & governance details*. | `claim_id`(s), per-requirement `met`/`unresolved`/`not_met` verbatim, `GOVERNED-CLAIMS.md#…` reference(s), any withheld claim ids + plain reason. No `crc_eligible`, no CRC-channel "may/must" prose. |
| 4 | In **Ask HRR** type *"what does governed knowledge say about copyrightability?"* → **Ask**. | One `POST` (`{mode:'question',question:…}`), **one** Anthropic classify call, **one** `lk_research` row. The response shows *"You asked: …"* verbatim, `intent_origin` "interpreted from your question", and (for the fixture) a `relevant_applicability_unresolved` topic: the proposition is shown in full, **What remains unresolved** lists `jurisdiction` + `human_contribution_description` with a note assigning each to the Human Reviewer, and **no** wording says the requirement is met/satisfied/established/cleared/compliant/sufficient. |
