...
**Last updated:** 2026-08-27 (created earlier this date during the original documentation-closeout pass, §5's "as of commit `18a308d`" reflects that original snapshot; §9 updated again later the same date, after that milestone's own §5 content, to record the Music/Artlist A-3 Living Knowledge domain-portability result — a documentation-only closeout, no further §5 milestones added by this later pass); §8/§10 updated 2026-09-11 to record the Synthetic-Person Natural-Language Extraction Robustness deferred item (item I), arising from the Bridge #1/#2 production UAT sequence — documentation-only, no runtime/governance/§5 change; §5/§8 updated 2026-09-18 to record the LK-DEMAND-2A→2E Material Demand durable evidence / governance-candidate foundation / admin-notification arc (item I) as closed, production-verified through 2E, and to record the LK-DEMAND-OPS-01 non-blocking operational finding (item J) — documentation-only, no runtime/governance change; §5/§8 updated 2026-09-20 to record the Third-Party Copyright Living Knowledge domain onboarding arc (candidate design → Dependency Semantics Challenge → FGR_022 → PM Adoption → CPR_028 → production representation → GoalCategory semantic-registration correction → production release `cae402e` → production UAT PASS) as closed (item K), and three non-blocking questioning/extraction observations from that UAT (items K/L/M) — documentation-only, no runtime/governance change, FGR_022/CPR_028 decisions not reopened
...
| I | **Material Demand durable evidence / governance-candidate foundation / admin notification (LK-DEMAND-2A→2E)** | A parallel, deliberately downstream-only pipeline (never read by Retrieval/BI/Track A/B/C/Composition/questioning/completion): explicit-goal-anchored Material Demand extraction with deterministic, exact-provenance occurrence identity (`kd-t{turn}-{sha256}`, no model-assigned id) → durable append-only evidence persistence (`crc_knowledge_demand_occurrences`, migration applied to production) → a bounded, human-created governance review-container + evidence-association foundation (`crc_knowledge_demand_governance_candidates`/`..._candidate_evidence`, migration applied to production; zero automatic candidate creation, zero subject/coverage/onboarding fields — see those migrations' own headers) → a best-effort, fail-open admin email fired only for durably, newly-inserted evidence rows (never on extraction alone, never re-firing on retry). Production UAT (2026-09-18, session `7fc98de9-a323-4b67-a7f8-8628ce1bde90`) confirmed two real observations ("Vimeo" turn 2, "publish it on Vimeo" turn 4) each produced a matching Supabase row and a corresponding admin email carrying the fixed governance-status disclaimer, with the first notification arriving independently of and before any customer results-email address was supplied — confirming the notification path is driven by the evidence/persistence path, not the CRC results-email flow. Governance-candidate tables are integrated and live but contain zero rows by design (candidate creation remains exclusively human-initiated; none has been created in production). | `02999ab`, `2cf2663`, `1638cd7`, `2b7b3d6`, `d6fb5b4`, `474e9e3`, `1be355c`, `01752a3`, `71ca140` |
| K | **Third-Party Copyright Living Knowledge domain onboarding + production UAT** | First LK domain onboarded end-to-end through the full governance chain in one arc: candidate design → an independent Dependency Semantics Challenge (rejected a proposed composite `unresolved_project_dependencies` entry after a decisive state-transition trace of `build-bounded-interpretation.ts` showed it could never reach a resolved state and unlocked no additional BI-permitted conclusion, landing on **D0 — zero dependencies**) → `FGR_022` (Formal Governance Review, independently re-derived, not inherited) → PM Adoption (`GOVERNED-CLAIMS.md`, `CLAIM-COPYRIGHT-US-THIRD-PARTY-OUTPUT-001-v1`, `Lifecycle: Adopted`) → `CPR_028` (CRC Publication Review, APPROVED, `CRC Eligible: Yes`) → production representation (new `GoalCategory` `third_party_copyright`, governed `TopicClaim` fixture: `provider_scope: null`, `jurisdiction equals United States` request-scope applicability gate only, `unresolved_project_dependencies: []`, explicit-goal-reachable only — no Track A, no `ContentPresenceCategory`, no `TopicRelationship`) → a follow-up correction closing a real gap this same arc surfaced (the extractor's model-facing `goal_category_hint` semantic-description block, distinct from mechanical enum registration, had no entry for `third_party_copyright` or the 3-day-prior `trademark` category — added both, plus a structural completeness test iterating the real `GOAL_CATEGORIES` array so a future mechanically-added category with no semantic entry fails automatically) → production release (merge commit `cae402e7e7d336f323d92a27def43f8a921715ef`, containing `3894f613` + `8c43d100`) → **production UAT PASS** (session `f196d6b4-fdd5-413b-ae0a-a60224aa85a1`, `runtime_commit: cae402e7e7d3`): natural-language opening formed an explicit, confirmed `third_party_copyright` `UserGoal` (`discovered_topic_occurrences: []`, `content_presence_mentions: []` — genuinely explicit, not discovered), exact-topic Retrieval selected the claim (`match_origin: exact_topic`, `relationship_id: null`), the US jurisdiction request-scope gate was satisfied from an explicit follow-up answer, zero dependencies carried through Retrieval/Plan/BI unchanged, BI reached `directly_relevant`, and the universal project-specific limitation survived into the final composed answer with no project-specific conclusion produced on infringement, protected/term status, ownership, authorization/license sufficiency, copying, substantial similarity, derivative-work status, fair use, or commercial clearance. Three non-blocking questioning/extraction observations from this same UAT are carried forward, not fixed, as items K/L/M in §8. | `3894f613c2e1e90feeb6d4acd4d5064b92ee6732`, `8c43d1000ec052282c0c91dc7ca549edc6a13efe`, merge `cae402e7e7d336f323d92a27def43f8a921715ef` |
...
| J | **LK-DEMAND-OPS-01 — related-observation notification multiplicity** | Observed in production (2026-09-18, session `7fc98de9-a323-4b67-a7f8-8628ce1bde90`): one CRC session produced two distinct durable Material Demand observations ("Vimeo" and "publish it on Vimeo") a human might recognize as concerning the same real-world subject — the architecture is not authorized to make that equivalence itself (see item I above). **Not a retry/idempotency defect** — the two observations have distinct source turns, raw text, and deterministic occurrence IDs; the admin-notification path correctly fired once per genuinely newly-inserted observation. Not approved: fuzzy deduplication, semantic normalization, canonicalization, notification-level entity resolution, or subject inference. Any future aggregation belongs in a later, separately-governed human-intake / subject-resolution milestone, not here. |
| K | **Third-Party Copyright UAT — redundant jurisdiction acquisition** | Observed in production (session `f196d6b4-fdd5-413b-ae0a-a60224aa85a1`): the opening turn already stated "United States," but `AssessmentJurisdictionMention` was not populated until CRC explicitly re-asked on a later turn and the user answered again. Did not cause the claim failure — applicability was correctly satisfied once the explicit answer arrived, and the claim's own request-scope gate is not the mechanism at fault. Not investigated or fixed in this pass; a future extraction/canonicalization or questioning-dedup milestone should re-examine whether an already-stated jurisdiction in the opening turn should be recognized without a re-ask. |
| L | **Third-Party Copyright UAT — extra questioning not owned by the new claim** | Observed in the same session: permission/clip-duration/transformation follow-up questions were asked, but these correspond to generic scoped-observation follow-up behavior, not to `CLAIM-COPYRIGHT-US-THIRD-PARTY-OUTPUT-001-v1`'s own governance — that claim carries zero dependencies and zero askability (§8-REVISED of its own candidate package; `FGR_022`; `CPR_028`), confirmed unchanged by this observation. Recorded for future questioning-provenance visibility work, not a defect in this claim's governance. |
| M | **Third-Party Copyright UAT — production-role question causation not fully traced** | Observed in the same session: a production-role question resulted in `human_contribution_description` becoming confirmed, but the persisted trace available for this UAT did not establish the precise governed cause of that specific question's selection. Not investigated further in this pass; not a blocker to this claim's closure (§5 item K) since it is orthogonal to `third_party_copyright`'s own explicit-only, zero-dependency, zero-askability path. |
