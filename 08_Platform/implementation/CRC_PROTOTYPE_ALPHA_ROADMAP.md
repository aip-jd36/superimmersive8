...
**LIVE-MODEL BATTERY RUN (2026-08-08).** JD authorized proceeding per `PHASE_7_PLANNING.md`'s approved asymmetric repeat counts. New eval-only harness (`live-model-deps.ts`, `run-real-model-dialogue-eval.ts`) wires the real Anthropic adapters into the unmodified orchestrator — zero frozen-module changes. Two-stage run (1-run smoke pass, smoke run counting as run #1 of the approved total, then the remaining approved runs): 26 total runs, 23 completed, 3 transient provider/schema failures (malformed JSON — same failure class as one Phase 6c instance). 172 live calls, 422,512 input / 64,968 output tokens, ~6.6s avg latency/call, estimated $2.24 (caveated rate). Full findings delivered to JD in chat, not duplicated here in full — see chat record for the complete structured report (smoke result, full-battery results, every divergence categorized, recurring-vs-isolated analysis, Rule 5 result, Gate 2 scope comparison, signal-lineage behavior, and the architecture-concern threshold assessment).

**Headline findings, recorded here for continuity:**
- **`diff.passed` is not a meaningful headline metric for the live battery** (0/23, expected and uninformative) — the live extractor mints its own candidate/mention/observation ids, which will essentially never match the hand-picked ids in each scenario's `expected` block. Semantic correctness (read from each run's actual final state) is the real signal, not the automated id-based diff.
- **`ambiguous_multi_surface_tool` failed to disambiguate Nano Banana in 3/3 live runs (100% consistent, not noisy).** Root cause identified, not a normalizer or lineage-fix defect: `anthropic-extractor.ts` calls the model with ONLY the current turn's raw text (`messages: [{ role: 'user', content: turn.text }]`) — no conversation history, no knowledge that CRC's prior turn asked a specific disambiguating question. In all 3 runs, turn 2's reply ("Through the API — I called it directly with my own developer key") was misclassified as a `workflow_role` update instead of a tool-mention correction, silently overwriting turn 1's "Designer" value (via `setWorkflowRole`'s plain-replacement semantics) and leaving the tool permanently unresolved. Classified as an **extraction failure** with a clear architectural explanation (stateless, no-history per-turn extraction), not a normalization or lineage-fix problem — `normalizeCandidate` was never even reached with a tool_mention candidate.
- **Finding 1's channel 1 (direct-statement hints) is confirmed dead in live mode**, exactly as flagged before the battery started: `anthropic-extractor.ts`'s own schema/prompt were never extended with the new hint fields, so `toCandidateObservation` never populates them regardless of what the model says. Only channel 2 (deterministic disambiguation-derived surface) can fire live, and even that was never actually exercised successfully in this battery due to the finding above.
- **Downstream consequence directly observed:** because live extraction can't populate `plan_tier`/`access_surface` even when directly stated ("team API plan," "both on team plans"), Constraint A — which only sees `StructuredUnderstanding`, not raw transcript — correctly-but-avoidably treats these as open gaps and asks about them (`rich_signal`, `mixed_multi_signal`). This is a real, material cost of the incomplete wiring, not merely cosmetic.
- **No over-extraction/invention observed anywhere in the battery** — `access_surface`/`plan_tier` never appeared confirmed without either a genuine direct-statement extraction success or the deterministic disambiguation channel; the dominant failure mode was under-extraction, not fabrication.
- **`full_opt_out` and the Rule 5 probe's core mechanism both replicated correctly live** — decline handling (Gate 1 `not_applicable_declined`, Gate 2 `DECLINE_BLOCKS_STABILITY`, Constraint B `USER_DECLINED_INTERVIEW`) and the once-per-interview disentangling cap (5/5 runs: first disentangling question asked, second independently-approved-by-Constraint-A-but-capped-by-Constraint-B) both matched expected behavior in every run.
- **Gate 2 phase/interview scope disagreement: 3/54 turns, all 3 within `ambiguous_multi_surface_tool`'s own repeated failure pattern, none changing final `completion_reason`.** No redesign warranted per JD's own threshold.
- **No genuine architecture-threshold finding.** Every material divergence traces to extraction-adapter completeness (a known, now better-understood gap) or expected model variance, not a defect in Extraction's core logic, normalization, mutation, either gate, boundary logic, lineage resolution, handoff assembly, or Constraint A's own decision framework.

**Not yet started: Phase 8.** Per explicit instruction, holding for JD's review of the full report before proceeding.
