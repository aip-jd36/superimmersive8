# Retrieval Engine — Architecture v1 (DRAFT — design exercise, not yet approved for implementation)

**Status:** Draft for review. Architecture design exercise only — no implementation authorized by this document.
...
`[PRINCIPLE]` Retrieval's only job, per PRD §3/§12: given the current structured understanding, determine what SI8 knowledge is relevant, updating silently after every turn, never influencing what the Interview Engine asks next. This is an architectural boundary, not a prompt instruction — genuinely independent systems or contexts, not a shared context window with a soft "don't let this leak" constraint.

**Owns:** matching the current `RetrievalHandoff` against SI8's knowledge sources (Platform Rights Matrix in the near term; Living Notebook / Platform Intelligence / reviewer-approved content per PRD §13 as those become indexable); filtering matches to only what is currently publication-eligible; producing a traceable, structured reference to each eligible match; re-running this after every turn Interview produces a new handoff for.
...
**Sparse handoffs.** Because Retrieval "updates silently after every turn," it runs against `RetrievalHandoff` snapshots throughout the conversation, not only at completion — including near-empty ones early on (`tools: []`, `scoped_observations: []`). Retrieval's algorithm must be well-defined and safe at every point on this spectrum, always producing a valid (possibly empty) result, never an error and never a placeholder result invented to avoid returning nothing.
...
**Step 3 — Resolve applicable publication scopes.** For each candidate row, read `CRC-Eligible`. Only `Yes` passes; `No` and `Pending` are both excluded, identically — the Matrix's own governing text is explicit that nothing is grandfathered in, "including rows with a Verified Status." For a compound row (a row carrying more than one sub-claim with independent eligibility, per the Living Notebook's claim-level-approval design — ElevenLabs' commercial-tiering-vs-consent split is the existing worked example), Retrieval must resolve the matched fact down to the specific sub-claim and check *that* sub-claim's eligibility only — a `Yes` on one sub-claim of a row never licenses surfacing a different, unapproved sub-claim of the same row.

**Step 4 — Determine eligible knowledge references.** The output of Steps 1–3: a set of `(source_fact, matrix_row_or_claim, CRC_Publication_Scope_text)` tuples — nothing here is yet Card content.
...
**Step 6 — Produce retrieval output.** Assemble the final result set: every `(source_fact, matrix_row_or_claim, CRC_Publication_Scope_text, source_metadata)` tuple that survived Steps 1–3, deduplicated by row/claim, unordered. See §5 for the exact shape.
...
```
{
  ...
  matrix_row_or_claim: <which row, and which sub-claim if compound>,
...
- **A row's `CRC-Eligible` was reset to `Pending` mid-project** (e.g., its `Status` moved off `Verified`, triggering the Living Notebook's own Reset principle). Not a Retrieval failure — a normal filtering outcome each time Retrieval runs. Implies Retrieval must always read eligibility fresh, never cache it across a stale window (§8).
...
- **A full negative-assertion suite**, mirroring Phase 5's handoff tests exactly: Retrieval's output must never contain `CRC-Eligible`, `CRC Publication Scope`, `CRC Decision Date`, `CRC Approver`, `SI8 Interpretation`, or any other internal Matrix field — tested as explicit assertions that fail loudly, not assumed correct by omission.
...
1. **Is `CRC Publication Scope` machine-readable, or only human-readable prose?** This is the single most load-bearing open question in this entire document. §3 Step 3 assumes claim-level eligibility can be looked up programmatically. The actual field, as documented (*"CRC may state only that..."*), reads as prose written for a human reviewer, not a structured, parseable schema. If it is prose-only today, Retrieval cannot be built as pure deterministic lookup against it without either a schema addition to the Matrix (structured, machine-parseable claim identifiers) or a parsing/interpretation step — which reopens the LLM question §7 just closed. This document does not resolve it, because resolving it means changing the Matrix's own schema, which belongs to whoever owns the Living Notebook, not to a Retrieval architecture document.
...
5. Any Platform Rights Matrix schema migration to resolve open question 1 above — a real, load-bearing gap this document surfaces, but proposing the actual migration is a decision for whoever owns that document, not something an engineering architecture document should unilaterally design.

---

## Verdict

**This document establishes a sound architectural foundation. It is not yet ready for full implementation planning — one concrete prerequisite decision needs to be made first.**

Everything in §§1–2, §4–6, §8–9, and the tool-matching path of §3 does not depend on anything unresolved and is ready to move into implementation planning now: the purpose boundaries, the input contract's handling of sparse/unresolved/declined/partial state, the matching philosophy, the output contract, failure-mode handling, and the deterministic-first testing strategy all stand on evidence already established in Prototype Alpha or on direct, careful reading of the frozen PRD and the Living Notebook's own governing documents.

What is not ready is §3 Step 3 as written — the eligibility-resolution step assumes a machine-readable `CRC Publication Scope`, and the evidence gathered while writing this document (Critique, open question 1) indicates that field is currently authored as prose for human reviewers, not as structured data. Proceeding straight to implementation without resolving this would either (a) silently build a parser/interpretation step nobody designed or evaluated, reintroducing exactly the kind of undesigned LLM dependency §7 argues against, or (b) implement Step 3 against an assumed schema that doesn't actually exist yet, discoverable only after code is written — precisely the failure mode Prototype Alpha's own discipline (deterministic-first, verify the contract before coding against it) exists to prevent.

This is the same kind of stop Prototype Alpha modeled repeatedly: name the dependency precisely, don't route around it, and let whoever owns the actual data (here, the Living Notebook / Matrix) make the call before implementation starts. Recommended next step is not more architecture — it's a short, targeted decision (with JD, as Matrix owner) on whether/how `CRC Publication Scope` becomes claim-level machine-readable, after which implementation planning for the rest of this document can proceed without revision.
