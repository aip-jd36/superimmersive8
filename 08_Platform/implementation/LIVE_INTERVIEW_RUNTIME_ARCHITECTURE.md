...
- **Phase 1→2: yes**, per §11's own stated rule (any workflow-relevant fact volunteered or asked for).
- **Phase 2→3: yes, but the PRD leaves the exact condition genuinely underdefined** — confirmed by direct re-reading: §7/§9 state Phase 2's purpose ("tools, assets, contributors, workflow") and one hard constraint (§11: *"Phase 2→3 is not gated on Gate 1 being met"*), but no dialogue library or flow diagram exists for Phase 1/2 the way §8 gives Phase 3. **`[PROTOTYPE ASSUMPTION — TO VALIDATE]`, adopted 2026-08-08 (JD decision, "Candidate B" from the design-review pass that preceded this):**

  > Phase 2 → Phase 3 once **both** hold:
  > 1. at least one active, resolved/canonical `ToolMention` exists; **and**
  > 2. at least one additional active, affirmative workflow-context fact exists — currently recognized as either a confirmed `workflow_role`, or at least one active `ScopedObservation`.
  >
  > Unresolved, unknown, declined, or superseded facts never count toward either condition.

  Deliberately does not require Gate 1, never calls `evaluateGate2`, has no dependency on Retrieval/Platform Intelligence/Knowledge Cards, invokes no LLM, and does not require every Phase 2 field (assets/contributors/workflow, in full) to be populated — closer to Phase 2's actual four-part scope than the bare tool-presence-only floor considered and set aside during design review, without requiring a phase tag on `ScopedObservation` that doesn't exist in the current type model. Known, disclosed imprecision: because `ScopedObservation` carries no phase tag, condition 2's second branch can be satisfied by an observation that is, in substance, Phase-3-shaped (e.g. surfaced early in a bundled answer) — an existing type-model limitation this rule inherits rather than introduces, shared with Gate 2's own already-documented phase/interview scope approximation.

  **`[REJECTED]` Candidate considered and explicitly set aside: reusing `evaluateGate2(previous, current, 'phase')` stability as the trigger.** On inspection, `gates.ts`'s `'phase'` evaluation scope does not represent conversational-phase stability — it means *Gate-1-relevant-field stability only* (tool identity + `intended_use`, deliberately excluding `access_surface`/`plan_tier`/`workflow_role`), a field-breadth distinction, not a per-conversational-phase one. Adopting it as the Phase 2→3 trigger would make phase progression substantively track Gate 1 readiness — creating an unintended Gate-1-proxy behavior that directly conflicts with §11's own explicit constraint. Rejected for this reason, not for cost — it would in fact have been the cheapest option to build.
...
**Phase progression:** a derived property, not a state machine — a pure `computePhase(SU, BoundaryState)` function mirroring Gate 1/2, returning `1 | 2 | 3` only. 1→2 is grounded in `INTERVIEW_ENGINE_ARCHITECTURE.md` §11; 3→Completion in `PRD_CRC_v1.0.md` §8's own flow diagram. **2→3 is genuinely underdefined by the PRD** — adopted as `[PROTOTYPE ASSUMPTION — TO VALIDATE]`, "Candidate B": at least one resolved `ToolMention` plus at least one of (confirmed `workflow_role`, an active `ScopedObservation`). Reusing `evaluateGate2(..., 'phase')` (Candidate C) was considered and explicitly rejected — it measures Gate-1-relevant-field stability, not conversational-phase stability, and would have created an unintended Gate-1-proxy in direct tension with §11. **Adopted runtime model, confirmed by dependency search across 8 areas (no meaningful dependency on `current_phase === 4` found anywhere): Phase 1 → Phase 2 → Phase 3 → Completion Check → Finalization** — "Phase 4" stays a PRD conceptual label for the gate-check behavior, never stored or traversed as an ordinary phase.
