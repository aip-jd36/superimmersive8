...
**Status:** `IMPLEMENTED / REBASED CLEAN ONTO main / PREVIEW PUSHED / CREDENTIALED VISUAL UAT + LIVE CLASSIFIER EVAL PENDING` — **Slices 1–7 COMPLETE** (CAH-4G.1…CAH-4G.7, 2026-09-10). **CAH-4G.7P:** the 19-commit series was rebased onto `origin/main` (`eb4641f`) with proven patch equivalence (all 19 `git range-diff` `=`, no conflicts) and zero new test failures vs a fresh `eb4641f` baseline; the rebased HEAD `e6ddc35` was pushed to `origin/work/cah-4g-current-main-integration` (new branch — `origin/main` untouched). **NOT deployed. The authenticated browser visual UAT + the bounded live classifier eval could NOT be run in the build environment (no Vercel auth / admin credentials / `ANTHROPIC_API_KEY` / fixture DB) — both are hard gates before merge to `main` / production.** This document + `HRR_GRI_TECHNICAL_DESIGN.md` (§S/§T/§U/§V) + `ADR-002` are the implementation contract. **The Living Knowledge tab is the converged Governed Research Interface: topic shortcuts + Ask HRR both POST `.../reviewer-lk/research` → `runAuditedHrrResearch()` (audit-before-content, fail-closed) → one `HrrResearchAnswer` → one `<HrrResearchAnswerView>` (consultative order hardened CAH-4G.7). Single-turn; raw question never persisted. The legacy `GET .../reviewer-lk` topic route is RETIRED (CAH-4G.7). CRC unchanged.** **Not `CLOSED`/`PRODUCTION-PROVEN`: production deployment + an authenticated visual UAT + the live classifier cost/quality eval are pending a credentialed preview/production environment.** The one genuinely-open item is `HRR_MAX_RESOLVED_TOPICS` (§T-4, kept at 2 — a usage experiment, not a blocker).
