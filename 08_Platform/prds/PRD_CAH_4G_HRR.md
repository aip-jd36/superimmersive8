...
2. **if — and only if — the question also names an explicit governed research topic**, runs governed research on that explicit portion and surfaces the relevant governed considerations + unresolved applicability;
...
**The interpretation call classifies + maps only.** Its structured output carries: `intent ∈ {research, assessment_judgment, unsupported}`; `resolved_topics: GoalCategory[]` (topics the *question* names — never a fabricated "closest topic"); `scope ∈ {informational, determination_request}`; `unresolved_ambiguity: <fixed phrases>`. It has **no free-text field** and **structurally cannot carry an answer, a claim, a legal statement, or a paraphrase of governed knowledge**. Schema-nonconforming output, empty `resolved_topics` for a `research` intent, or low confidence → **fail closed to `unsupported`** (fixed "try a topic" copy; no retrieval).

**May free-form resolve multiple topics?** Yes — a question can legitimately name two governed topics ("does commercial-use permission tell me anything about copyright ownership?" resolves `commercial_use` + `copyright_ownership`). Each resolved topic runs the governed pipeline; the answer groups by topic. (Open question §T-4: cap on topic count.)
