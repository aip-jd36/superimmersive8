...
**Status:** three milestones under this one product spec.
...
- **CAH-4F.1 — Reviewer Workspace Shell** — **INTEGRATED / PRODUCTION DEPLOYED** (2026-09-10); shell IA confirmed operational in the CAH-4F.2 production UAT, though the dedicated §16 script was not run as its own session (a formal CAH-4F.1 closure decision has not been separately made). Integrated to `origin/main` by rebasing the two reviewed commits onto `9203162` (unrelated `governance(crc)` commit, zero file overlap — `range-diff` confirmed identical patch) and fast-forwarding `main` to **`b0e067f`** (feat `f76dc85`, docs `b0e067f`). Vercel deployed `b0e067f` to the `si8-creator-portal` **Production** environment (`app.superimmersive8.com`) — build `success`, per the Vercel/GitHub deployment record (deployment `6363227586`). Pre-merge validation from the rebased tree: reviewer-shell/lk/context 14 suites / 301 tests pass; `tsc --noEmit` clean; `next build` exit 0; full-suite failing set byte-identical to the `9203162` baseline (20 suites / 77 tests, established fresh), +35 new passing, zero new failures. The dedicated §16 CAH-4F.1 UAT script was not run as a separate session, but the three-region shell + closable inspector + Living Knowledge / Linked CRC Context tab separation + "no top-of-page stacked layout" were all directly exercised and confirmed operational in production during the 2026-09-10 CAH-4F.2 UAT (see §17a). Introduces the approved three-region shell (`ASSESSMENT NAVIGATION | ASSESSMENT WORK SURFACE | REVIEWER RESOURCES inspector`), moving all CAH-4F semantic work into a closable right-side inspector. **Supersedes the CAH-4F layout only.** No route/selector/eligibility/applicability/audit/schema/migration change; `WorkbookClient` change = one line (`h-screen`→`h-full`). Open questions resolved in §13. As-built design: `REVIEWER_RESOURCES_ARCHITECTURE.md` §1, §5, §7, §11 (responsive), §16 (UAT). (The feature branch `origin/work/cah-4f-reviewer-resources` remains at the pre-rebase SHAs `2b40780`/`56373cc` — a historical record of what was reviewed; not force-updated.)
- **CAH-4F.2 — Contextual Inspector Coordination** — **CLOSED / PRODUCTION-PROVEN** (2026-09-10). Integrated to `origin/main` by rebasing the two reviewed commits onto `74a8f63` (unrelated Envato/Epidemic Living Knowledge activation — zero file overlap; `range-diff` confirmed identical patch) and fast-forwarding `main` to `f29b0fb` (feat `4f1969d`), then docs status commits `7c70f3c` → CLOSED. Vercel deployed to the `si8-creator-portal` **Production** environment (`app.superimmersive8.com`) — build `success` (deployment `6363990353`, `sha = f29b0fb`). **Engineering proof:** 15 reviewer suites / 320 tests pass, `tsc --noEmit` clean, `next build` exit 0, full-suite failing set byte-identical to a fresh `74a8f63` baseline (20 suites / 77 tests), zero new failures — established twice (pre-change local at `e63ceda`, and fresh from the rebased tree at `74a8f63`); `git diff` confirms no route / selector / eligibility / applicability / retrieval / bounded-interpretation / audit / schema / migration / API change and no change to any Reviewer Resources component. **Production visual proof** (PM/user authenticated manual UAT, 2026-09-10, `app.superimmersive8.com`, fixture `CA-RLK-2a PROD SMOKE — internal synthetic` / `ASSESS-007-2026-09-07`, ~1440px): Reviewer Resources CLOSED → `NAV | WORKBOOK | WORKBOOK CONTEXT` (Guidance/Submission/Evidence visible, Workbook primary); Reviewer Resources OPEN → `NAV | WORKBOOK | REVIEWER RESOURCES` (Workbook context aside visually gone, Workbook reclaimed the rail width, Reviewer Resources visually separate, Living Knowledge + Linked CRC Context tabs, no top-of-page stacked layout, the CAH-4F.1 four-region density gone); CLOSED again → Workbook context aside returned with the previously-selected Guidance tab still selected, same section, no visible Workbook reset (visual/session-state confirmation — **not** an independent DB-invariance proof); one explicit `Copyright ownership` Living Knowledge look-up rendered a single governed result inside the coordinated inspector with the Workbook still visible + primary — proposition primary, explicit "Applicability" heading ("Established for this submission." / "No additional applicability requirements."), "Context used for this look-up" (Jurisdiction: Global; "Submission-derived inputs to retrieval — not assessment evidence."), separate "Evidence limitations / unresolved requirements" and "Provenance and governance details", **no** `crc_eligible` / "CRC channel: Yes", **no** raw "CRC may state…" prose, **no** evidence-promotion / finding / conclusion affordance, reviewer framing "Reference materials to support your review — not assessment evidence." intact. **Audit:** CAH-4E already production-proved the `lk_research` audit contract; CAH-4F.2 does not change that path (unchanged by diff + tests). This UAT session produced no new `crc_context_access_events` query and none is claimed. **Detailed UAT record:** `REVIEWER_RESOURCES_ARCHITECTURE.md` §17a. **Non-blocking presentation-polish observation (not a CAH-4F.2 issue, not scheduled):** the "Context used for this look-up" section renders `Resolved tools —` even when no resolved tools are available — could later be omitted when empty. Production visual UAT of CAH-4F.1 confirmed the shell architecture but exposed **four-region density** at ordinary desktop width (`NAV | WORKBOOK | WORKBOOK CONTEXT ASIDE | REVIEWER RESOURCES` all visible at ~1440px → the assessment work surface unnecessarily cramped). CAH-4F.2 makes the contextual right rail hold **one surface at a time at adjacent widths**: Reviewer Resources CLOSED → `NAV | WORKBOOK | WORKBOOK CONTEXT`; Reviewer Resources OPEN → `NAV | WORKBOOK | REVIEWER RESOURCES`; on close the Workbook context aside returns with the same `rightTab`. Mechanism: `ReviewerShell` derives one layout-only boolean (`workbookContextAsideHidden = resourcesAvailable && inspectorOpen && adjacent`) and publishes it through a domain-neutral `workspace-layout-context` around the opaque Workbook child; `WorkbookClient` consumes only that boolean and CSS-hides (never unmounts) its own Guidance/Submission/Evidence aside. `rightTab` stays entirely Workbook-owned. **This is a UX / perceptual-clarity mechanism, not an authority boundary** — data ownership, service boundaries, audit paths, bounded interpretation and mutation boundaries are unchanged and continue to enforce authority separation independently. No route/selector/eligibility/applicability/audit/schema/migration change; no change to any Reviewer Resources component; `page.tsx` structurally unchanged. Uniform mutual exclusion at every adjacent width incl. 1920px (no ">=1800 show-both" mode). Drawer behaviour below the breakpoint unchanged. Files: **new** `workspace-layout-context.tsx`; `ReviewerShell.tsx` (+import, +derived boolean, +provider wrap); `WorkbookClient.tsx` (+hook, +one conditional class on the existing `<aside>`). Validation history: pre-change local (parent `e63ceda`) and again fresh from the rebased tree (parent `74a8f63`) — 15 reviewer suites / 320 tests, `tsc` clean, `next build` exit 0, zero new failures both times. As-built: `REVIEWER_RESOURCES_ARCHITECTURE.md` §1, §11a, §17. (The feature branch `origin/work/cah-4f-reviewer-resources` was rebased; its pre-rebase SHAs `e6adcb2`/`4b10fb3` are superseded by the integrated `4f1969d`/`f29b0fb`.)
