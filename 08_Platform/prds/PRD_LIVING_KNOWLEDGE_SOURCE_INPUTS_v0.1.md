...
**Second concrete example (2026-08-17, Phase 0.5 — Third-Party Source Assets CRC routing/representation design, `08_Platform/implementation/THIRD_PARTY_SOURCE_ASSETS_ROUTING_ARCHITECTURE.md`):** the same production conversation that originally motivated this PRD's §3 Getty Images example produced a second, related but not identical, instance of the underlying gap this section names. There, a user's `commercial_use`/`copyright_ownership` goals were already captured when they incidentally disclosed having sourced images from Getty — a disclosure, not a further goal statement. Recorded here as an open question for whoever eventually scopes this capability, not resolved by this cross-reference: whether "causing previously-uncaptured governed knowledge to become retrieval-eligible at all, from a project fact CRC already holds" (that design's "Path B") is the same capability as this section's original motivating case ("composing several ALREADY-retrieved, already-eligible principles differently based on what the user described"), or a related-but-narrower precursor to it. Both share the same non-negotiable boundary already stated above: recognizing relevance is bounded and retrieval-adjacent; declaring sufficiency, or inventing a goal/relationship the user never stated, is not.
