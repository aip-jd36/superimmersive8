...
| CRC Publication Scope | The exact claim approved or withheld, and why. See § CRC-Eligible Governance |
...
**Status convention (added 2026-08-05):** row-level Status reflects the overall confidence of the record. Any material sub-claim with a different confidence level must be stated explicitly in the relevant field and must not be left implicit in the overall Status label. Added after the 8-platform verification pass (`notebook/MATRIX-LEARNINGS.md`) found this exact gap recurring independently at half the platforms checked — a single Status word repeatedly had to carry a row where one sub-claim was confirmed and another wasn't (e.g. Sora: discontinuation dates Verified, historical ToS terms Unconfirmed; Runway: core ToS Verified, one Enterprise sub-claim not independently checked). This is a documented convention, not a new field — the distinction is expressed in prose within the relevant field, the same way the fact/interpretation split above is.
...
**Claim-level approval (added 2026-08-05):** `CRC-Eligible` approves a specific statement, not the entire row. This matters most for compound rows — a row can carry both a cleanly Verified claim and a separate Needs-Reverification sub-claim (ElevenLabs is the worked example: commercial-tiering terms Verified, consent-specific provisions not). `CRC-Eligible = Yes` on such a row can only ever scope to the Verified claim; the unverified sub-claim stays excluded regardless of what the rest of the row decides, and never becomes eligible by association. The Matrix stays row-based — this isn't a redesign into sub-records — but `CRC Publication Scope` (below) must state exactly which claim CRC is permitted to state, so the row itself never has to be read as "fully cleared" just because `CRC-Eligible` reads `Yes`.

**Additional fields, added alongside `CRC-Eligible` (added 2026-08-05; this field renamed from "CRC Publication Note" to "CRC Publication Scope" the same day):** the original name invited a real misreading — "approved" internally means "JD scoped this specific statement for release," but a later reader could easily read it as "SI8 approved this platform" or "SI8 approved this workflow," exactly the kind of institutional-opinion inference CRC exists to avoid. The field was never recording an approval; it records the boundary of what CRC is permitted to say. Renamed accordingly, content and purpose unchanged.
- **CRC Publication Scope** — the exact statement CRC is permitted to make, stated as scope rather than verdict (*"CRC may state only that..."*, not *"Approved: ..."*). For a `No`, write `None — withheld because...` rather than the reason alone, so the field still reads as scope even when nothing is permitted. For a compound row, states explicitly which sub-claim is in scope and which is excluded. This is the field that actually satisfies the Publication Policy's traceability requirement — `CRC-Eligible` alone is not enough of a record.
...
**Reset principle (claim-level, sharpened 2026-08-08 alongside the CRC Claims sub-table below — the underlying principle is unchanged, only its scope of effect is now stated explicitly):** `CRC-Eligible` resets to `Pending` whenever anything changes that could affect a claim's factual correctness or its suitability for unsupervised publication — scoped to the claim(s) actually affected, not automatically the whole row.
- **Claim-scoped reset (the default):** a change to content that is clearly *about* one specific claim resets only that claim. ElevenLabs is the worked example: editing the consent/likeness language resets `elevenlabs-voice-consent` alone and does not touch `elevenlabs-commercial-tiering`.
- **Row-wide reset:** a change to a field the Matrix does not currently split by claim — `Status`, `Source`, `Last Verified`, `Source Wording/Confirmed Fact`, `SI8 Interpretation`, `Known Restrictions`, or any other row-level field — resets every claim under that row, since the affected fact underlies all of them equally. Per the Lifecycle rule above, `Status` moving away from `Verified` for any reason is inherently row-wide (Status itself has no claim granularity) and always resets every claim on that row.
- **Ambiguous impact defaults to row-wide, not claim-scoped.** If it's genuinely unclear which claim a change affects, treat it as row-wide — the conservative direction is requiring re-review, not silently preserving a possibly-stale eligibility decision.
- **Unrelated claims do not reset merely because a sibling claim changed.** A claim-scoped reset touches only the claim(s) it's actually about, never the row's other claims by association.

Stated as a principle rather than a fixed checklist, for the same reason as before: a closed list invites exactly the kind of gap already found once here — the first version of this rule enumerated content fields but missed Status itself moving away from Verified via the routine 6-month staleness trigger. Treat the field list above as illustrative, not exhaustive: if a future edit changes what a row or claim means without matching one of these examples, the reset still applies, scoped per the rule above. `CRC Publication Scope`, `CRC Candidate Statement`, and `CRC Decision Date` reset alongside `CRC-Eligible`, at whatever scope (claim or row) the triggering change itself was — a stale note describing a claim that no longer matches its current content is worse than a blank one.
...
**`CRC-Eligible`, `CRC Publication Scope`, `CRC Candidate Statement`, `CRC Decision Date`, and `CRC Approver` all remain internal and must never appear in a CRC Knowledge Card or any user-facing output.** `CRC Publication Scope` is an input to what the Projection Layer is allowed to build a Card from — it is not itself Card content. `CRC Candidate Statement` is drafted reference text, not a standing offer to publish: its existence is never itself authorization. A consumer may only treat a claim's `CRC Candidate Statement` as usable source material when that *same* claim's `CRC-Eligible` reads `Yes`, checked at time of use — never on the presence of drafted text alone, and never inherited from a sibling claim on the same row (§ CRC Claims sub-table below covers exactly this: the two fields live in the same claim record specifically so they can never be read apart from each other).
...
#### CRC Claims sub-table (added 2026-08-08)

Added when Retrieval Engine architecture design (`08_Platform/implementation/RETRIEVAL_ENGINE_ARCHITECTURE.md`) surfaced a real gap: claim-level approval was already the *governing principle* (§ Claim-level approval, above), but the fields recording it — `CRC-Eligible`, `CRC Publication Scope`, `CRC Candidate Statement`, `CRC Decision Date`, `CRC Approver` — were recorded as prose interleaved inside a single row-level cell for compound rows (bolded "Claim A: ... Claim B: ..." labels with no stable, parseable boundary between them). That's readable by a human reviewer but not addressable by a deterministic consumer. This section formalizes the storage shape; it does not change what any of those fields *mean*, and it is not itself a re-review of any existing decision.

**Shape:** each Matrix row carries a `CRC Claims` sub-table, directly beneath the row's own field-value table, with one sub-table row per claim:

| Claim ID | CRC-Eligible | CRC Publication Scope | CRC Candidate Statement | CRC Decision Date | CRC Approver |
|---|---|---|---|---|---|

**`Claim ID`** is the new field. It is the stable key a deterministic consumer (Retrieval) matches on — never parsed for meaning, never inferred, always an exact-match lookup.
- **Single-claim row** (the common case): `Claim ID` is simply the platform's existing canonical tool identifier, unchanged — `runway-gen3`, `kling`, `pika`, `gemini-api`, `gemini-consumer-app`, etc. No renaming. A single-claim row's `CRC Claims` sub-table has exactly one row, and every value in it is a direct copy of what the row already stated before this convention existed.
- **Compound row** (a row whose sub-claims carry independent `CRC-Eligible` dispositions — ElevenLabs' commercial-tiering-vs-voice-consent split remains the sole worked example): `Claim ID` is namespaced `<canonical-identifier>-<claim-slug>` — `elevenlabs-commercial-tiering`, `elevenlabs-voice-consent`. One sub-table row per claim.

**`CRC Publication Scope` and `CRC Candidate Statement` remain human-readable governance/audit prose, authored under `notebook/CRC-PUBLICATION-POLICY.md` exactly as before.** Moving them into the claims sub-table is a storage-shape change only — it does not make them machine-parseable, and nothing downstream is permitted to parse or interpret their meaning. A deterministic consumer may read `CRC-Eligible` to decide *whether* a claim is in scope, and may pass `CRC Publication Scope`'s text forward *verbatim* once a claim is in scope — it must never attempt to derive meaning from the prose itself. `CRC Candidate Statement` moved into the same sub-table for the same reason `CRC Publication Scope` needed one: it already had separately-authored claim-level content (ElevenLabs' "Claim A only" note), and leaving it in a top-level prose field would have preserved the identical claim-boundary problem for whichever system consumes it next (Projection Layer, not yet designed) — this is claim-*ownership* normalization, not Projection design, and does not add Candidate Statement to any consumer's contract that didn't already need equivalent content.

**This section does not extend `CRC-Eligible`'s own semantics.** `Yes`/`No`/`Pending`, the Lifecycle rule, the Reset principle, and the claim-level-approval principle above all apply identically to a `Claim ID` row as they did to the row-level fields being replaced — nothing about *how eligibility is decided* changes here, only *where the decision is recorded*.
