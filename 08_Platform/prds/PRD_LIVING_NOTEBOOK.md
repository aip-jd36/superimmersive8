...
**`CRC-Eligible`, `CRC Publication Scope`, `CRC Decision Date`, and `CRC Approver` all remain internal and must never appear in a CRC Knowledge Card or any user-facing output.** `CRC Publication Scope` is an input to what the Projection Layer is allowed to build a Card from — it is not itself Card content.
