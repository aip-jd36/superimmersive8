...
| **REVIEWER RESOURCES — Architecture** | `08_Platform/implementation/REVIEWER_RESOURCES_ARCHITECTURE.md` | ACTIVE | The as-built internal design of the reviewer-side surfaces (CAH-4B reviewer CRC context, CAH-4C transcript access, CAH-4E Reviewer Living Knowledge — component/data flow, system boundaries, retrieval/applicability/UI-projection/audit contracts, firewall test map) **plus** the CAH-4F design surface (proposed files, data/API = none, fail-closed, test strategy, prohibited couplings, CAH-4G forward note). As-built basis `9fa6d1c`. Sep 10, 2026. |
| **PRD: CAH-4F Reviewer Resources** | `08_Platform/prds/PRD_CAH_4F_REVIEWER_RESOURCES.md` | DRAFT | Product contract for the CAH-4F milestone (not built): a bounded "Reviewer Resources" area beside the workbook grouping Living Knowledge + Linked CRC Context; LK Context Semantics (reviewer-readable topic labels, explicit Applicability heading, neutral styling, "Context used for this look-up", proposition-first, progressive provenance disclosure, no raw CRC-channel prose as reviewer authority, `crc_eligible` hidden by default, no silent promotion to evidence/findings). Authority boundaries §4; explicit CAH-4G (conversational LK) future boundary §11. Companion decision: `lib/reviewer-lk/ADR-001-reviewer-resources-authority-boundary.md`. Sep 10, 2026. |
