...
### 3o. Assessment Report-Binding Lifecycle Fix (Jul 17-18, 2026)
- [x] Root cause identified — ✓ `ASSESS-001-2026-07-10` vs `ASSESS-005-2026-07-12`: the same Cloud World assessment carried two different IDs from two uncoordinated numbering systems — a legacy non-atomic `submissions.assess_id` field (auto-generated on every first workbook page load, by simple row-count, no sequence) vs. the canonical atomic-sequence `assessments.assessment_number`. See `06_Operations/assessments/ASSESS-005-2026-07-12/README.md` for the full reconciliation.
- [x] Report-binding lifecycle series shipped (3 commits: schema/service invariants, Generate Report wiring, `/sign` enforcement) — ✓ Legacy ID generator removed entirely; canonical `assessment_number` now allocated once, at Generate Report time (get-or-create + explicit outcome sync, never silent); new `submissions.report_pdf_assessment_id` binding column ties each uploaded report PDF to the exact assessment it was generated for, validated by ID match + SHA-256 re-check at `/sign`; stale-report invalidation reverts `REPORT_GENERATED` → `DRAFT` whenever workbook data changes post-generation, forcing regeneration before re-signing
- [x] Public Verification Page exposure gate (pushed independently, first) — ✓ `findAssessmentForVerification()` now requires `processing_status = 'DELIVERED'`; `SIGNED`-but-undelivered assessments — previously publicly visible to anyone with the URL — now resolve identically to a nonexistent assessment number
- [x] Staging validation checklist — ✓ `08_Platform/implementation/REPORT_BINDING_STAGING_CHECKLIST.md`, 10 sections (idempotency, concurrency, stale-report reuse, artifact binding, missing-assessment errors, retry-after-failure, manual overrides, public leakage, end-to-end integrity). Strengthened via a second architecture review pass before execution; executed for real against a disposable test assessment before push
- [x] Production bug found + fixed during staging validation — ✓ Public assessment page had no `export const dynamic` directive; Next.js cached its first-ever render per deployment and served that frozen snapshot regardless of later `processing_status` changes (confirmed via Supabase API logs showing zero repeat queries for the route). Fixed with `export const dynamic = 'force-dynamic'`; verified live against real production data (Cloud World, `ASSESS-005-2026-07-12`, marked `DELIVERED` for the test)
- [x] Internal error observability added — ✓ `findAssessmentForVerification()` now logs structured errors (code/message/details/hint) for genuine query failures, while treating PostgREST's `PGRST116` (normal "no rows" signal) as silent — externally indistinguishable to the public page, internally observable in logs
- [x] Audited all other dynamic-segment and public pages for the same caching gap — ✓ no second instance found; every other server-rendered page either redirects or calls `cookies()` via `requireAdmin()`/the server Supabase client, which auto-forces dynamic rendering
- [x] Known limitation tracked, not blocking — ✓ `08_Platform/app/lib/assessments/ADR-003-manual-pdf-upload-provenance.md`: manual PDF re-uploads prove artifact identity + post-upload integrity but not content provenance (that the file was actually generated from current workbook state). Recommended future architecture documented (system-managed artifact versions, `SYSTEM_GENERATED`/`MANUAL_OVERRIDE` provenance)
- [ ] Two smaller follow-ups flagged, not yet scheduled: no retry-on-conflict for concurrent Generate Report clicks (losing request gets a raw 500, not a graceful response); no audit trail (uploader/timestamp/reason) for manual PDF override uploads

### 3p. Reviewer Manual v0.2 — Domain H Corroboration Requirement (Jul 20, 2026)
- [x] Root cause identified — ✓ Cloud World (ASSESS-005-2026-07-12) scored the top outcome/High confidence on uncorroborated self-attested authorship; traced to two gaps in Reviewer Manual v0.1: Domain H's evidence hierarchy treated a detailed written account alone as sufficient for "Verified," and the outcome-decision logic never gated the top outcome/High confidence on Domain H being Verified at all
- [x] Reviewer Manual v0.2 issued — ✓ Domain H now requires a corroborating artifact (prompt log, project file, storyboard, brief) for "Verified"; uncorroborated narrative caps at "Partially Verified." Domain H added as a required-Verified gate for the top outcome and High confidence. v0.1 preserved unmodified as an immutable historical record (header-only change: Status → SUPERSEDED, pointer to v0.2) — Cloud World's stored `methodology_version` is never retroactively changed. New governance sentence added (Part 7): issued assessments are not retroactively re-evaluated when a later Manual version is published
- [x] Version Bump Requirement added to Part 7 — ✓ formalizes that any substantive Part 4/Part 5 change requires a Manual version bump + a matching update to `METHODOLOGY_VERSION` in `service.ts` in the same change set — this is the fix for the mechanism that let this drift happen in the first place
- [x] `METHODOLOGY_VERSION` updated + exported — ✓ `08_Platform/app/lib/assessments/service.ts`; confirmed via full write-path audit (every `updateAssessment()` call site checked) that `methodology_version` is set exactly once, automatically, at creation — no code path or admin UI field allows a reviewer to manually select or override it
- [x] Repo-wide propagation — ✓ 15 hardcoded "v0.1" references found and classified: 10 updated across 8 files (including two live bugs — report-boilerplate text in `Section7Brief.tsx` and missing corroboration guidance in the live reviewer tool `guidance.ts`, both would have let new assessments be reviewed/reported under stale v0.1 rules despite the Manual itself being fixed), 5 preserved as accurate historical record (migrations, PRDs, superseded docs, Cloud World's own report source)
- [ ] **Tracked follow-up — not forgotten:** `06_Operations/reviewer-workbook/SI8-Control-Classification-Note-v1.0.md` contains an H02 Methodology Note that uses Cloud World's uncorroborated narrative as its own worked example of *sufficient* evidence — a substantive conflict with Reviewer Manual v0.2's Domain H rule, not just a stale cross-reference. Needs a deliberate revision pass, not a mechanical version-string swap. Deliberately left untouched during the v0.2 change rather than silently edited.
- [ ] `SI8-Reviewer-Manual-v0.2-Candidate-Improvements.md` naming collision — pre-existing file (5 unrelated hypotheses, no Domain H overlap) now shares "v0.2" naming with the real, shipped `SI8-Reviewer-Manual-v0.2.md`. Recommend renaming to "v0.3 Candidate Improvements."
- [ ] `08_Platform/app/lib/assessments/IMPLEMENTATION-NOTE.md` references a `buildZoneA()` C2PA-building function that no longer exists in the codebase (confirmed via repo-wide search) — stale note, not a functional risk, but should be corrected next time that file is touched

### 3q. CRC Results Gate — Email Delivery Model + crc_leads (Aug 14, 2026)
- [x] Mid-conversation email gate fully retired — ✓ `email_required`, the turn-4 Discovery/fallback trigger, pending-message resume, and `decline_email` all removed; CRC now always completes naturally before any identity ask. Zero changes to Interview Engine, Gate 1/2, Constraint A/B, Model 4, Retrieval, or Projection — confirmed via `git diff` file list (no file under `lib/interview-engine/`, `lib/retrieval-engine/`, or `lib/projection-layer/` touched)
- [x] Results-gate/teaser replaces it — ✓ deterministic zero-invented teaser (`consideration_count` = `knowledge_items.length`, fixed copy otherwise, singular/plural handled), gate copy per PM revision (no "Work email" — "Email address" instead)
- [x] Email delivery, not browser unlock — ✓ the core invariant: full `ProjectionOutput` (`understood_summary`/`knowledge_items`/`opening_line`) is never returned to the browser for a non-grandfathered session, at any point, before or after email submission — enforced by one shared `buildCompleteResponseFields()` helper used by both GET and POST, not per-route discipline. Result is delivered by real email instead (`lib/emails.ts` → `sendCrcResultsEmail`, existing verified `noreply@superimmersive8.com` Resend integration, no new provider)
- [x] `crc_leads` — ✓ thin durable cross-session identity (`id`/`email`/`email_normalized` UNIQUE/`created_at`/`last_seen_at`), no CRM fields, no cached session-count/first/latest (derived by query). Atomic `upsert_crc_lead()` Postgres function (`INSERT ... ON CONFLICT`), not the Supabase JS client's `.upsert()` (confirmed unreliable for conflict detection elsewhere in this project, 2026-08-09)
- [x] Atomic send-ownership — ✓ `claim_crc_result_send()` Postgres function using `SELECT ... FOR UPDATE` to serialize concurrent claims at the database level (double-click/browser-retry/refresh cannot double-send); enforces 60s explicit-resend cooldown, max 2 explicit resends, max 3 distinct recipients per session — all config-driven (`CRC_CONFIG`), not hardcoded. Resend SDK v3.5.0 has no idempotency-key option in its typed API, so atomicity lives entirely server-side, before any provider call
- [x] DB-failure policy — ✓ lead-persistence failure blocks the send entirely (Resend never called, result never exposed); a provider-accepted send survives a subsequent outcome-recording failure (user still sees "sent," inconsistency logged loudly via `logPilotEvent`, never silently swallowed and never misreported as failed)
- [x] Delivery status precision — ✓ `results_email_status`: `pending`/`accepted`/`failed`/`unknown` only — no `delivered`/`verified` state exists (would require Resend bounce/delivery webhooks, deliberately deferred to phase 2; today's schema doesn't block adding them later). `results_email_accepted_at` means exactly "Resend's API confirmed acceptance," never delivery/open/ownership
- [x] `results_gate_shown` analytics event — ✓ idempotent per session (mirrors `commercial_assurance_bridge_shown`'s existing once-guard pattern), no email/transcript/Projection content ever in `event_data`
- [x] Grandfathering — ✓ `CRC_CONFIG.resultsGateLaunchedAt` marker; sessions before it keep the full-in-browser-result behavior unconditionally, exactly as before this milestone
- [x] Historical backfill — ✓ `lib/crc-engine/scripts/backfill-crc-leads.ts`, one-time, idempotent; groups existing `crc_sessions.email` rows by normalized address, replays `upsert_crc_lead()` chronologically, links sessions — never resends, never rewrites transcripts/`identity_source`
- [x] **Two production bugs found and fixed during this session's own live validation, before sign-off** — ✓ (1) `resultsGateLaunchedAt` was initially set to midnight of the deploy *day* rather than the deploy *moment*; since this milestone shipped on the same calendar day as ~55 real pre-existing sessions, that would have retroactively newly-gated every one of them. Caught by directly querying a real historical session's live API response, fixed, redeployed, re-confirmed live (`grandfathered:true` restored). (2) The backfill script replayed by `crc_sessions.created_at` instead of `email_captured_at` — wrong for a session whose email was corrected after creation, producing one lead with `last_seen_at` earlier than its own `created_at`. Fixed, re-run (idempotent), verified corrected
- [x] Live validation against real production data (not just design review) — ✓ fresh `internal_test` session, natural completion with a real Discovery takeaway, teaser/gate correct, zero model calls on email submission (confirmed via absence of `retryable_failure`/`persistence_error` pilot events), 2 real emails sent via Resend and **confirmed received with correct per-session content** in JD's own inbox, duplicate/mixed-case resubmission idempotent, cooldown proven via a tightly-controlled back-to-back RPC test, change-email created a new lead + new send, a second independent session correctly deduped to an existing lead (not a duplicate), grandfathered historical session unaffected — both directions confirmed simultaneously via direct API queries
- [x] Migration applied via Supabase SQL Editor (no direct Postgres/Management API access from this environment — service-role key is PostgREST-only) — copy-paste-from-chat repeatedly corrupted the long comment header (confirmed unrelated to the SQL itself via a working isolated test statement); resolved by pushing the exact file bytes to the clipboard directly from disk (`Get-Content -Raw | Set-Clipboard`), bypassing chat rendering entirely
- [x] Tests — ✓ 59 new/updated across 6 files; full suite 713 passing (2 pre-existing unrelated failures, confirmed via `git stash` prior to this milestone); `tsc --noEmit` clean; `next build` exit 0
- [ ] Not yet exercised live (unit-tested + SQL-reviewed only): DB-failure-before-send path, provider-timeout mapping, max-2-explicit-resend cap, max-3-distinct-recipient cap — would require either simulating a real outage or sending several more real emails for coverage already proven two other ways
- [ ] Resend bounce/delivery webhooks — deliberately deferred (phase 2), per approved design
...
### 3r. Music Scenario A / Artlist A-3 — First Living Knowledge Domain-Portability Proof (Aug 27, 2026)
- [x] Full lifecycle carried through for one Music-domain claim — ✓ Artlist evidence acquisition (human Print-to-PDF capture, automated fetch failed) → durable preservation + checksum → 10-claim FGR (Formal Governance Review) → Adoption → CPR (CRC Publication Review, initial disposition: WITHHOLD, runtime-verification prerequisite missing) → Artlist provider registration (generic registry extension) → synthetic-eligible runtime canary (bounded, never mutating real governance) → explicit human CRC-publication decision for exactly one claim, `CLAIM-MUSIC-ARTLIST-PROJECT-LICENSE-DURATION-001-v1` ("A-3") → real runtime fixture representation → deterministic Retrieval/Track A/Bounded Interpretation/Projection verification → integration → production UAT (real user conversation, reproduced against captured input on the exact deployed commit)
- [x] Narrow, correctly-scoped conclusion — ✓ **A-3 demonstrated end-to-end production portability through the existing generic CRC architecture; this does not mean "Music is portable."** The other 9 Music Scenario A claims remain `Lifecycle: Adopted`, `CRC Approver: PENDING` — ungoverned by this milestone, unaffected
- [x] Cross-domain bleed lesson — ✓ registering Artlist would have made two pre-existing, unrelated stock-media claims (`provider_scope: null`) newly, unintentionally reachable — caught by a diagnostic before registration, corrected via the existing supersession mechanism (v1→v2), not new architecture
- [x] Generic LK Cross-Domain Bleed Preflight tool shipped — ✓ `08_Platform/app/lib/crc-engine/cross-domain-bleed-preflight.ts` — deterministic, read-only, reuses real `providerScopeMatches`/`deriveDiscoveredTopicOccurrences`/`normalizeCandidate` (never reimplements them); reports facts for human review only, never a governance verdict; scoped specifically to provider-shaped LK onboarding, not a universal domain preflight
- [x] Full technical detail, governance-vs-runtime-portability distinction, synthetic-canary precedent, refresh-lifecycle status, and next-portability-experiment recommendation — ✓ `08_Platform/implementation/CRC_CURRENT_STATE.md` §9 (updated same milestone) — not duplicated here
- [ ] Refresh fail-closed policy (what CRC should do when governed knowledge passes its expected refresh point without successful re-verification) — explicit PM/governance decision still required before any refresh scheduling/automation

### 3s. Validate Commercial Readiness Decision Layer (Strategic Hypothesis — Aug 29, 2026)

**Status: VALIDATION, not implementation.** See "ACTIVE STRATEGIC HYPOTHESIS" section above for full context (Decision Engine thesis, three-surface model, lifecycle hypothesis, falsification conditions). This gap tracks the three parallel validation workstreams — none authorize building the Decision Engine, an API, or changing CRC/Commercial Assurance's current product definitions.

- [ ] **Capability — Project-State Decision Delta:** construct ~5 adversarial paired cases (identical artifact, one changed underlying project-state/evidence fact) across provider/account eligibility, source/reference assets, likeness/synthetic-use consent, licensing scope, missing evidence. Benchmark where practical against ZebraTruth, Creation Rights, a frontier general-purpose model, and SI8's proposed methodology. Core metric: does the system correctly change its conclusion when the hidden fact changes while the artifact doesn't?
- [ ] **Demand — Customer/Gatekeeper Value:** use the paired cases as concrete discovery stimuli (never ask abstractly "would you buy a Decision Engine?") with CPs, agencies/Business Affairs, brand legal/procurement, insurance/E&O participants, and Anchor/production-platform partners where accessible. Test whether the distinction changes a real decision, who owns the problem, who has budget, and whether pre-production checking beats post-production review.
- [ ] **Economics — Machine/Human Boundary:** classify each benchmark decision M (machine-determinable) / E (evidence required) / H (human judgment) / X (external escalation); estimate machine-resolvable share, human-review rate/minutes, inference/data costs, and escalation rate — to determine whether this resembles scalable infrastructure or technology-enabled professional services. Do not assume the answer.

**Do NOT** update `PRICING-STRATEGY-v3.0.md`, `TECHNICAL_ARCHITECTURE.md`, or any frozen/current PRD based on this workstream. Do NOT design or build a generalized API.
...
| **Taiwan AI Subsidy Program (2026 MODA)** | `01_Business/research/TAIWAN-AI-SUBSIDY-PROGRAM-2026-MODA.md` | ACTIVE | Digital Industries Administration's 開發產業AI便利工具及補助計畫: NT$6.7M subsidy cap (50% of a ≥NT$13.4M project), eligibility (incl. foreign-entity/PRC-capital exclusion), timeline (announces 2026-09-01, closes 2026-09-30), review criteria. Sent directly by Alice Feng (Anchor Film, B164) Aug 25, confirming this is the program she referenced on the Aug 21 call. Lead-applicant math worked out same day: SI8's Taiwan entity (NT$1.55M capital) capped near NT$1.55M subsidy as lead vs. ~NT$6M if Anchor leads instead — informing a proposed Anchor-leads/SI8-as-partner structure. Aug 25, 2026 |
...
| **Sales Pipeline** | `03_Sales/SALES-PIPELINE.md` | ACTIVE | Stage-by-stage funnel view (HubSpot/Pipedrive style) — 10 stages, realigned Jul 18, 2026 to match SE engineering's `lead_responses.pipeline_stage` DB enum exactly: Lead Responded → 1st Mtg Scheduled → 1st Mtg Taken → Evaluating → Proposal Made → Negotiating → Nurturing → Unqualified → Closed Won → Closed Lost. Lead Responded internally sub-groups by temperature (Call Requested / Warm / Replied) to preserve prioritization. CRM.md's own separate stage vocabulary was not touched. Updated via `/dripify-report` Step 7. |
| **ICP Definitions** | `03_Sales/ICP-DEFINITIONS.md` | ACTIVE | Restructured 2026-07-19 following the v5 institutional pivot: **ICP 1 = Agency** (direct buyer, single assessment), **ICP 2 = Legal** (demand generator/cascade, was old "ICP 3"), **ICP 3 = Line Producer/EP — Film** (was old "ICP 2," now formally DEFERRED — wrong channel, wrong product shape, wrong buyer ecosystem; hold all outreach). Use for call prep, campaign targeting, and alias briefing. |
| **Lead Sweep vs. v5 Pivot** | `03_Sales/LEAD-SWEEP-V5-PIVOT-2026-07-18.md` | ACTIVE | Full hot/warm/cold lead sweep against the v5 institutional positioning + ICP validity discussion (FACTS/HYPOTHESES/OPEN QUESTIONS format) + a portable brainstorming prompt for external AI agents. Source doc for the 2026-07-19 ICP restructuring and pricing model decisions. |
...
| **Standing Encore Signal Test — How-To** | `03_Sales/standing-encore/SE-SIGNAL-TEST-HOWTO.md` | ACTIVE | Filed verbatim from Standing Encore (JD's own managed-outreach company — see `03_Sales/PRESENTATION-JUN4.md`). Fixed-methodology product: 500 sends, one audience held constant, message as the only variable, FIT/RESONANCE/ACTION scoring, ~3-week turnaround to an Intelligence Report. Do not edit — reference doc only. Jul 30, 2026. |
| **Standing Encore Signal Test Log** | `03_Sales/standing-encore/SIGNAL-TEST-LOG.md` | ACTIVE | Tracks actual SI8 Signal Tests run through Standing Encore — audience, hypothesis, message arms, results, and what it changes in SI8's own messaging. Explicitly a separate system from `03_Sales/outreach/SPLIT-TEST-LOG.md` (SI8's own Dripify-backed pain-discovery tests) — see the comparison table at the top of the file. SE-001 (Creative Directors) in design as of Jul 30, 2026. |
...
| **Commercial Risks of AI Video — External Evidence** | `01_Business/research/COMMERCIAL-RISKS-AI-VIDEO-EXTERNAL-EVIDENCE-2026.md` | ACTIVE | External legal/insurance evidence base (Thoolie, Akker Insurance, Rodriques Law, SNR Films, Kelly Insurance, Beverly Boy) mapping 6 documented commercial risks (no clean chain of title, non-commercial AI tool licenses, weak/absent copyright on AI-only work, training-data contamination, likeness/synthetic-performer exposure, the E&O coverage gap) 1:1 onto SI8's assessment domains. Primary Thoolie source fetched and verified Jul 25, 2026. Explicitly flagged as FEATURE-FILM / ICP-3 evidence — research ammunition for the deferred Line Producer/EP track, not the near-term Agency/Enterprise motion. Also independently confirms the Domain H corroboration fix in Reviewer Manual v0.2 ("prompts alone ≠ authorship"). |
| **Commercial Risks of AI Advertising — External Evidence** | `01_Business/research/COMMERCIAL-RISKS-AI-ADVERTISING-EXTERNAL-EVIDENCE-2026.md` | ACTIVE | Companion to the AI Video evidence doc, but for the LIVE ICP (Agency/Legal, not the deferred film track). 7 law-firm sources (Hunton, Varnum, Influencers-Time, Trestle, Ogilvy, Cummings & Cummings, Taft), 2026. Centerpiece finding, verified word-for-word against Hunton's original (Jun 3, 2026): "the retailer, not the third party, will be subject to an enforcement action" — the documented legal backbone of SI8's brand-legal-cascades-to-agency thesis. Adds a 7th risk category unique to advertising (FTC/deceptive-advertising exposure) not present in the film evidence. Second independent confirmation of the Domain H corroboration fix ("prompts alone ≠ authorship," now in an advertising context). |
...
| **Reviewer Manual v0.2** | `06_Operations/reviewer-workbook/SI8-Reviewer-Manual-v0.2.md` | ACTIVE | 7-part internal manual: Philosophy (independence, evidence, uncertainty) → Framework (7 domains, 5 outcomes) → Workflow (Steps 0–6) → Domain Guidance (A/R/H/I/L/T/D, each with decision logic + evidence calibration) → Synthesis → Report Guidance → Governance. Jul 20, 2026. Domain H (Human Creative Contribution) now requires a corroborating artifact — not just a detailed submitter narrative — to reach "Verified"; Domain H added as a required-Verified gate for the top outcome and High confidence. Added the Version Bump Requirement to Part 7. `METHODOLOGY_VERSION` constant in `service.ts` updated to match. |
| Reviewer Manual v0.1 | `06_Operations/reviewer-workbook/SI8-Reviewer-Manual-v0.1.md` | SUPERSEDED | Superseded by v0.2 (Jul 20, 2026). Preserved unmodified as the historical methodology under which prior assessments (e.g. ASSESS-005-2026-07-12, Cloud World) were issued — never retroactively changed. |
| **Report Delivery SOP v0.1** | `06_Operations/reviewer-workbook/SI8-Report-Delivery-SOP-v0.1.md` | ACTIVE | 6-step delivery procedure: populate template → pre-delivery checklist → PDF export → delivery email (3 subject line variants, body template by outcome) → record-keeping (workbook archive, CRM update, pipeline update) → 48hr follow-up. Jul 5, 2026. |
| **Reviewer Manual v0.2 Candidate Improvements** | `06_Operations/reviewer-workbook/SI8-Reviewer-Manual-v0.2-Candidate-Improvements.md` | ACTIVE | Hypothesis log for proposed Manual improvements not yet validated by real assessments. 5 hypotheses: T02 deployment-context calibration, R01 subscription date gap rule, R01 personal-tier handling, L voice model provenance, Evidence Guide export instructions. Promotes to Manual only after real assessment evidence. Jul 6, 2026. |
...
| **Assessments Archive** | `06_Operations/assessments/` | ACTIVE | One subdirectory per assessment (numbered by ASSESS-ID). `assessment-zero/` = system validation dossier. `post-assessment-reviews/` = all PARs. README documents structure and naming convention. Jul 6, 2026. `ASSESS-005-2026-07-12/README.md` added Jul 17 — reconciles the legacy `assess_id` field's `ASSESS-001-2026-07-10` against the canonical `ASSESS-005-2026-07-12`; both IDs are the same Cloud World assessment. |
...
| **PRD: Assessment Service v1.0** | `08_Platform/prds/PRD_ASSESSMENT_SERVICE_v1.0.md` | ACTIVE | Engineering implementation spec: Assessment Registry as authoritative source, separate institutional/processing status, ProvenanceProvider abstraction (Numbers as first impl), Verification Page at `/assessment/{assessment_number}`, confidence PDF-only, report hash internal-only in v1. Implemented and live Jul 12, 2026. First live assessment: ASSESS-005-2026-07-12 (Cloud World). See PRD Implementation Notes for deviations. Report-binding lifecycle fix (ID mismatch root cause + public exposure gate + `force-dynamic` cache fix) shipped Jul 17-18, 2026 — see Execution Gaps §3o. |
| **Report Binding Staging Checklist** | `08_Platform/implementation/REPORT_BINDING_STAGING_CHECKLIST.md` | ACTIVE | 10-section validation plan for the report-binding lifecycle series: idempotency, concurrency, stale-report reuse, artifact binding, missing-assessment errors, retry-after-failure, manual overrides, public leakage, end-to-end integrity. Executed Jul 17, 2026 before push. |
| **ADR-003: Manual PDF Upload Provenance** | `08_Platform/app/lib/assessments/ADR-003-manual-pdf-upload-provenance.md` | ACTIVE | Known limitation, not blocking: manual PDF re-uploads prove artifact identity + post-upload integrity, not content provenance (that the file was actually generated from current workbook state). Recommended future architecture: system-managed artifact versions, `SYSTEM_GENERATED`/`MANUAL_OVERRIDE` provenance. Jul 17, 2026. |
...
| **Pricing Strategy v3.0** | `01_Business/pricing/PRICING-STRATEGY-v3.0.md` | ACTIVE | Post-v5-pivot model (2026-07-19): three motions — Legacy self-serve (unpublished fallback), Agency (~$499 floor hypothesis, ad hoc/discount-lever pricing, unlisted pricing page shared on request), Enterprise (custom-scoped governance program, vision-sold, v1 delivery scoped as pilot — explicit guardrail against promising SLA/dashboard features not yet deliverable). |
| Pricing Strategy v2.0 | `01_Business/pricing/PRICING-STRATEGY-v2.0.md` | SUPERSEDED | Two-tier CaaS pricing: Creator Record $29 + SI8 Certified $499 (March 2026) — superseded by v3.0 institutional model |
| Pricing Strategy v1.0 | `01_Business/pricing/PRICING-STRATEGY-v1.0.md` | ARCHIVED | v3 licensing tiers (Rights Agency model) |
| Rights Playbook v0.1 | `06_Operations/legal/rights-playbook/versions/v0.1.md` | SUPERSEDED | Rights vetting — sales asset for buyers. Superseded by v0.2 (Feb 2026) — kept as historical record, not corrected retroactively. |
| **Rights Playbook v0.2** | `06_Operations/legal/rights-playbook/versions/v0.2.md` | ACTIVE | Current rights-vetting sales asset. Adds: Colorado AI Act "rebuttable presumption" safe harbor, USCO copyrightability (human authorship) requirements, DMCA CMI preservation requirements, EU AI Act Article 50 transparency obligations, NY synthetic performer disclosure law, Adobe competitive limitations clarified. Feb 2026. Found unindexed and cross-referenced only as "done" in a Mar 2026 checklist line, not in this table, during the Aug 6, 2026 governance review. |
...
| **PRD: Living Notebook** | `08_Platform/prds/PRD_LIVING_NOTEBOOK.md` | ACTIVE — v1.0 architecture frozen | Spec for the four-document Living Notebook (Platform Rights Matrix / SI8 Positions / Edge Cases / Pending Questions) — markdown-first, git-native, companion to (not replacement for) the Reviewer Manual. Includes "Default State: Uncertainty" governance principle and Freeze Notice. Jul 30, 2026, revised Aug 1-2, 2026; Aug 5-6, 2026: added CRC-Eligible Governance (claim-level publication approval, `CRC-Eligible`/`CRC Publication Scope`/`CRC Decision Date`/`CRC Approver` fields on Platform Rights Matrix rows) and the Status convention (row-level Status vs. per-field sub-claim confidence, stated explicitly not implied). |
| **PRD: Living Knowledge Source Inputs** | `08_Platform/prds/PRD_LIVING_KNOWLEDGE_SOURCE_INPUTS_v0.1.md` | ACTIVE — directional/future architecture | Distinct from the Living Notebook above — this PRD directs the future of `GOVERNED-CLAIMS.md`'s own research/demand/governance loop, not the four-document notebook. Four demand sources (CRC user demand, automated source monitoring, Commercial Assurance reviewer demand, SI8 strategic research) converging on one pipeline: demand → knowledge need → agent-assisted research → candidate knowledge → human governance → governed knowledge → product surfaces. Establishes as durable principle: Adoption ≠ CRC publication; discovery is not governance; no automatic CRC publication regardless of research confidence; Global jurisdiction means structural, not exhaustively-verified. Generalizes directly from CLAIM-COPY-004-v1's own completed lifecycle (first claim to go Adopted → Reviewer-only → CRC-eligible via a full governance + source-hardening pass). Does not authorize schema changes, reviewer UI, agent deployment, or knowledge-gap queue implementation. Aug 17, 2026. |
| **PRD: CRC (Commercial Readiness Check)** | `08_Platform/prds/PRD_CRC_v1.0.md` | ACTIVE — v1.0 frozen | Free, AI-assisted commercial-readiness conversation — the free-tier front door replacing the $29 Creator Record in SI8's marketed funnel. Two-engine architecture (Interview Engine / Knowledge Projection + Retrieval, deliberately independent so retrieval never drives what the interview asks), Gate 1 (Minimum Understanding) + Gate 2 (Understanding Stability) completion logic, Phase 3 (Post-Production Discovery) six-dialogue normative library, mutable scoped-observation data model. CRC decomposes a described workflow into verified educational claims about each platform; SI8 synthesizes workflow evidence into commercial-readiness judgment — downstream of SI8 Principle 4, not a replacement for it. Frozen Aug 5, 2026. |
| **CRC Implementation Risks** | `08_Platform/implementation/CRC_IMPLEMENTATION_RISKS.md` | ACTIVE | Companion to the CRC PRD, deliberately kept separate so empirical uncertainty doesn't clutter the frozen product spec: LLM behavioral adherence to the six normative dialogues, thin-retrieval-at-launch risk, mid-conversation correction handling. Five empirical questions framed as the actual prototype test plan. Aug 5, 2026. |
| **CRC Current State** | `08_Platform/implementation/CRC_CURRENT_STATE.md` | ACTIVE | The living cross-cutting CRC/Living Knowledge status doc — closed/accepted milestones (ToolMention persistence, selector-before-completion, question budget=6, jurisdiction follow-up capping, selector-attempt observability, no-visibility continuation investigated/no-change), deferred backlog, product boundary + layering + Track A/B/C + governance invariants restated, and the next active workstream (Living Knowledge Domain Portability diagnostic). Not a replacement for the frozen PRD or the per-layer architecture docs it indexes. Aug 27, 2026. |
| **Commercial Assurance Architecture Index** | `08_Platform/implementation/COMMERCIAL_ASSURANCE_ARCHITECTURE_INDEX.md` | ACTIVE | Pointer doc (duplicates nothing) — where to read about CRC, Living Knowledge, the CRC → Commercial Assurance handoff (CAH-4x), the three Reviewer authority surfaces (Assessment Workbook / Reviewer Living Knowledge / Linked CRC Context), the reviewer-side access audit, and the workbook/assessment authority boundary. Start here for reviewer-side or CAH-handoff engineering. Sep 10, 2026. |
| **REVIEWER RESOURCES — Architecture** | `08_Platform/implementation/REVIEWER_RESOURCES_ARCHITECTURE.md` | ACTIVE | As-built internal design of the reviewer-side surfaces CAH-4B…4F.2 (CAH-4B reviewer CRC context, CAH-4C transcript access, CAH-4E Reviewer Living Knowledge, CAH-4F LK Context Semantics, **CAH-4F.1 Reviewer Workspace Shell**, **CAH-4F.2 Contextual Inspector Coordination**). §1 = component tree (shell + provider + inspector); §5 = LK card contract; §7 = CAH-4F.1 as-built file table; §11 = responsive breakpoint (measured, `ADJACENT_MIN_PX = 1440`); §11a = CAH-4F.2 contextual-rail time-sharing (one contextual surface at a time at adjacent widths, uniform incl. 1920px); §16 / §17 = CAH-4F.1 / CAH-4F.2 manual UAT scripts. Data-flow internals at `9fa6d1c`; CAH-4F.1 shell integrated at `b0e067f`; **CAH-4F.2 integrated to `origin/main` = `f29b0fb` (feat `4f1969d`, rebased onto `74a8f63` + fast-forwarded), Vercel-deployed to `si8-creator-portal` Production (`app.superimmersive8.com`), and CLOSED / PRODUCTION-PROVEN 2026-09-10 after a passing PM/user authenticated visual UAT — see §17a (CLOSED rail-state observations + real `Copyright ownership` LK look-up render; engineering vs. production visual evidence kept separate; audit path unchanged, CAH-4E already proved the `lk_research` contract).** CAH-4F.1 shell IA also confirmed operational during that same UAT. Sep 10, 2026. |
| **PRD: CAH-4G HRR / Governed Research Interface** | `08_Platform/prds/PRD_CAH_4G_HRR.md` · design + implementation contract `08_Platform/implementation/HRR_GRI_TECHNICAL_DESIGN.md` (§S freeze, §T resolved OQs, §U slices, §V UAT) · decisions `08_Platform/app/lib/reviewer-lk/ADR-002-governed-research-interface.md` + `ADR-003-hrr-conversation-context.md` · conversational design `08_Platform/implementation/HRR_CONVERSATIONAL_ARCHITECTURE.md` | **`CAH-4G.10 SLICE A — CLOSED / PRODUCTION-PROVEN`** · **`CAH-4G CONVERSATIONAL HRR — VISIBLE THREAD PRODUCTION-PROVEN / BOUNDED CONVERSATIONAL CONTEXT NOT STARTED`** · **`CAH-4G.11 — FOLLOW-UP DISCOVERY / ARCHITECTURE VALIDATION — DESIGN / EVIDENCE COLLECTION`** · **`CAH-4G.12 — TOPIC-SCOPED HRR RESEARCH SESSION ARCHITECTURE / DESIGN`** — **ACCEPTED** · **`CAH-4G.13 — SESSION-FOCUSED PRODUCTION DISCOVERY / PRE-REGISTERED / PM EXECUTION PENDING`** (`08_Platform/implementation/HRR_SESSION_DISCOVERY.md` — pre-registered discovery table + a gated, one-turn-at-a-time PM script against the internal synthetic fixture; CLI has no authenticated browser session and did not attempt to bypass one; CAH-4G.14 implementation remains NOT AUTHORIZED) (`08_Platform/implementation/HRR_RESEARCH_SESSION_CONTRACT.md` — PM product decision: HRR is a bounded, topic-anchored Research Session focused on one issue at a time, never an unlimited conversational assistant over the whole workbook; the one authoritative session/focus contract, with pointers from the other CAH-4G docs; bounded session context NOT IMPLEMENTED; Workbook Research Log FUTURE / NOT AUTHORIZED) (`08_Platform/implementation/HRR_FOLLOWUP_DISCOVERY.md` — generic follow-up taxonomy, pre-registered PM discovery script, evidence threshold for Slice B; no runtime change; Slice B remains NOT STARTED / NOT AUTHORIZED) (CAH-4G.1…CAH-4G.7 + CAH-4G.7P + CAH-4G.8 + CAH-4G.9 design + CAH-4G.10 Slice A + CAH-4G.10P + CAH-4G.10I + CAH-4G.10C, Sep 10–11 2026). **CAH-4G.10P (2026-09-11)** proved a **DEPLOYMENT GAP** — Slice A (`bea6f2d`/`f4d33ec`) was reviewed locally but on no origin ref. **CAH-4G.10I (2026-09-11)** rebased the reviewed lineage onto `origin/main` (`d06c668`; 2 zero-overlap CRC-only drift commits) — `git range-diff` all `=` (patch-identical) — then **fast-forward-pushed `d06c668 → 4b9ee5a`** (no force); `tsc` clean, `next build` exit 0, zero new test failures vs a fresh `d06c668` baseline; Vercel auto-deploy observed. **CAH-4G.10C (2026-09-11) — production UAT:** a PM-performed authenticated production UAT against the internal synthetic fixture `CA-RLK-2a PROD SMOKE` / `ASSESS-007-2026-09-07` **passed all 10 acceptance checks** — heading "Research conversation" / action "Clear conversation" confirmed live; a topic pick + two successive free-form questions (Copyright ownership → Copyrightability → Commercial use) all remained visible together (append-only, not replace); composer cleared each time; Living Knowledge ↔ Linked CRC Context tab-switch and Reviewer Resources close/reopen both preserved the thread; leaving/re-entering the workbook cleared it (the V1 client-memory contract — not a defect); "Clear conversation" reset the thread with no assessment-state change. Full evidence: `HRR_CONVERSATIONAL_ARCHITECTURE.md §FF`. **What is closed: the VISIBLE thread only.** Non-blocking observations recorded, not fixed (§HH): thread density on long answers in the narrow rail; the reviewer's question rendering twice (visible turn + `HrrResearchAnswerView`'s own echo); whether HRR research should survive ordinary workbook navigation is an open **future** product question. **Slice B (bounded conversational context / referent resolution) remains NOT STARTED / NOT AUTHORIZED** — each question is still researched fully independently, no `prior_context`, no transcript to the model. Prior status (CAH-4G.10, 2026-09-11) — Slice A implemented + reviewed LOCALLY: `ReviewerLkLookup` rewritten to an append-only research thread (`app/admin/submissions/[id]/review/hrr-thread.ts` — a local pure `useReducer` reducer; LOCAL HRR mechanics, no generic conversation framework). Successive topic picks + questions stack; asking a second question no longer replaces the first. **VISIBLE, not CONTEXTUAL** — every free-form question is still classified independently from its own text alone; **no `prior_context`**; no prior turn reaches the classifier / retrieval / applicability / BI / composition / audit. Topic shortcuts still 0 model calls; free-form still ≤1 classify-only; request body byte-unchanged; audit unchanged (one `lk_research` per governed-research action; "Clear conversation" makes no server call); no persistence / storage / schema / migration; no CRC change. `tsc` clean, `next build` exit 0, full-suite failing set byte-identical to a fresh `08f20e9` baseline (zero new failures; +36 passing). As-built: `HRR_CONVERSATIONAL_ARCHITECTURE.md §BB`. Slice B (bounded enum-only follow-up referent) + Slice C not started. **CAH-4G.9 (design only, NO runtime code):** PM production UAT PASSED the single-turn experience (initial surface, topic shortcut, `copyright_ownership`, unresolved `copyrightability`, free-form NL classification, semantic + authority boundaries) but found the **single-turn model too visible** — a second free-form question replaces the first; Human Reviewers expect a **conversational research thread** (interaction model like CRC). Not an unsafe defect / not a rollback / a material V1 research-experience gap. Architecture: `HRR_CONVERSATIONAL_ARCHITECTURE.md` + `ADR-003` — visible thread = append-only **client-only** UI (no persistence, refresh-clears); the follow-up classifier gets a **bounded enum-only `{topic, scope, bi_status}` referent** (`HrrThreadContext`), **never the transcript** (CRC's own `buildUserMessageContent` — bounded `[Context:]` lines, never transcript-to-LLM — is the precedent, traced from source); **one-model-call ceiling preserved** (topic 0 / free-form ≤1 / follow-up ≤1); **no audit change, no schema/migration, no CRC change**; reviewer conversation is never a project fact or evidence. GRI formalised as a *pattern*, not a shared module; shared surface = a new generic client shell (`components/conversation/`). Hypothetical / jurisdiction-qualifier research is **out of V1** (needs PM + applicability-architecture review). Implementation = Slices A→B→C, none begun. — **Slice 1 (BI input adaptation boundary) COMPLETE (CAH-4G.1, Sep 10, 2026)** · **Slice 2 (classifier + authority gate) COMPLETE (CAH-4G.2)** — `lib/reviewer-lk/interpret-research-intent.*` + `hrr-authority-gate.ts` (intent entry only). · **Slice 3 (converged governed-research pipeline) COMPLETE after the 3A semantic correction (CAH-4G.3 + CAH-4G.3A, Sep 10, 2026)** — NEW `lib/hrr/`: `runHrrResearch()` (PURE — one downstream path for the topic chip and the free-form classifier+gate; reviewer selection/eligibility/applicability byte-unchanged; `not_met` claims excluded from BI → `does_not_apply[]`; raw reviewer question never enters selection/applicability/BI; BI stays the semantic ceiling) + `researchIntentToBiIntent` / `reviewerClaimToBiResult` adapters + `HrrResearchResult`. `BiResult` introduced with **zero CRC call-site/test churn**. **CAH-4G.3A:** deterministic applicability is a generic upstream semantic input to BI (`BiResult.applicability`, optional — CRC byte-unchanged) → an applicability-`unresolved` governed proposition is interpreted `relevant_applicability_unresolved`, **never `directly_relevant`**; proposition still visible; unresolved requirement preserved; no new BI status; no composition workaround; durable principle in `ADR-002`. All changes proven byte-for-byte behavior-neutral for CRC; `tsc` clean, `next build` exit 0, zero new test failures. **Slice 4 (deterministic consultative composition / projection) COMPLETE (CAH-4G.4, Sep 10, 2026)** — `lib/hrr/project-hrr-research-answer.ts`: `projectHrrResearchAnswer(HrrResearchResult) → HrrResearchAnswer`, **deterministic, no second LLM call**. Consultant-like answer: fixed `orientation` per BI status + verbatim governed propositions + verbatim `summary_blocks` + mechanical deduplicated applicability rollup + `unresolved_inputs` made useful (each assigned to the Human Reviewer) + separate `does_not_apply` block + fixed `boundary_note`. Authored prose = one exported enumerable constant (`HRR_ANSWER_TEMPLATES`); grounding A–E test-enforced (`__tests__/hrr/hrr-projection.test.ts` — every rendered substantive string traces to a governed statement / BI block / fixed template / mechanical enumeration; a false premise in the question is never echoed as fact). Assessment-authority refusal and governed research are **two structurally separate lanes**; `relevant_applicability_unresolved` is wording-locked away from established/resolved/satisfied/cleared/compliant/sufficient; ordering is deterministic input order (no governed materiality signal — documented limitation). No `rules.ts` / CRC-composition change. **Slice 5 (audit enrichment + audit-before-content) COMPLETE (CAH-4G.5, Sep 10, 2026 — Option A, NO migration)** — `lib/hrr/project-hrr-audit-record.ts` (PURE: `projectHrrAuditRecord` → `{actorUserId, submissionId}` when governed research occurred, `null` otherwise) + `lib/hrr-audit/run-audited-hrr-research.ts` (the one impure boundary: pure pipeline → `projectHrrAuditRecord` → `await recordReviewerLkAccess` → **`HrrAuditNotRecordedError` on failure** → answer only on success). **Reuses the CAH-4E `lk_research` access contract UNCHANGED** — source inspection superseded the frozen Option B: the deployed `lk_research` row records no topic/claim, and the frozen Option-B placement (structured writer in `lib/reviewer-lk/repository.ts`) breaks `authority-firewall.test.ts`. An HRR audit record means only "an authorized Human Reviewer used governed research on this submission at this time" — never assessment evidence / finding / control / sufficiency / outcome / sign-off / reliance. Authority-only ("should I approve this?") + unsupported → **no audit event**; multi-topic / mixed → **one row per reviewer action**; raw question never persisted or hashed; applicability / BI status / claim ids deliberately not persisted. Test: `__tests__/hrr/audited-hrr-research.test.ts`. **Slice 6 (Reviewer Resources HRR interface + audited runtime integration) COMPLETE (CAH-4G.6, Sep 10, 2026)** — the Living Knowledge tab is the converged Governed Research Interface. ONE route `POST /api/admin/submissions/[id]/reviewer-lk/research` accepts `{mode:'topic_pick',topic}` or `{mode:'question',question}` (strict input contract — `messages[]`/`history`/`session_id` → 400, length-bounded at `HRR_QUESTION_MAX_LENGTH`, no client-supplied actor/authority facts); both modes call `runAuditedHrrResearch()` (topic path = 0 model calls; free-form = 1 classify-only, fails closed; audit-throw → 503 zero content). ONE renderer `HrrResearchAnswerView.tsx` for both modes — no topic/question split, no topic-specific branch; orientation prominent, unresolved before provenance, assessment-authority boundary a distinct callout, empty sections omitted, neutral styling (no pass/fail, no ✓/✗), no `crc_eligible`/`crc_publication_scope`, no promotion control. Rewritten `ReviewerLkLookup.tsx`: topic chips run research on click, `<textarea>` Ask HRR, ONE current response replaces the previous, race-guarded, **raw question never persisted to storage/URL/logs — page reload clears it**, no conversation memory. CAH-4F.1/4F.2 shell + inspector + Linked-CRC separation + responsive behaviour unchanged; no migration; no CRC change. Legacy `GET .../reviewer-lk` retained non-UI pending Slice-7 retirement. Tests: `hrr-research-route.test.ts` (new), `reviewer-resources-presentation.test.ts` (rewritten). **Slice 7 (visual UAT + composition hardening + integration closeout) COMPLETE (CAH-4G.7, Sep 10, 2026)** — server-rendered-markup review of the 7 answer cases (real components + real pipeline; no authenticated browser path in the build env) → renderer-only consultative hardening in `HrrResearchAnswerView.tsx` (no BI / composition / contract change; `bi_summary_blocks` still verbatim): "what remains unresolved" now leads (right after orientation); the two per-topic provenance disclosures collapse to one; per-claim requirement/ref re-lists dropped; "Not yet established" rollup group dropped — the unresolved case went 4071→3143 chars and its material issue is now first. **Legacy `GET .../reviewer-lk` route + `route-and-audit.test.ts` + `projectReviewerLkResult` + `ReviewerLkLookupResult` RETIRED** (zero runtime consumers; fail-closed audit contract transferred to `hrr-research-route.test.ts`). `HRR_MAX_RESOLVED_TOPICS` kept at 2. `project-reviewer-claims.ts` → leaf `REVIEWER_LK_FRAMING` module. **Status: `IMPLEMENTED / INTEGRATION PENDING` — NOT `CLOSED`/`PRODUCTION-PROVEN`. NOT DEPLOYED. Authenticated visual UAT (`REVIEWER_RESOURCES_ARCHITECTURE.md §18`) + live classifier cost/quality eval are pending a credentialed preview/production environment.** Zero new test failures (baseline `0c33910`); `tsc` clean; `next build` exit 0. **CAH-4G.7P (Sep 10 2026):** the 19-commit CAH-4G series was rebased onto `origin/main` (`eb4641f`) — all 19 commits `git range-diff` patch-identical, no conflicts (zero file overlap with the Firefly/Veo drift); post-rebase full-suite failing set byte-identical to a fresh `eb4641f` baseline (zero new failures), `tsc` clean, `next build` exit 0. Rebased HEAD `e6ddc35`/`261debf` pushed to **`origin/work/cah-4g-current-main-integration`**; Preview credentialed UAT blocked by a known Cloudflare Turnstile issue in Preview mode. **CAH-4G.8 — Controlled Production Integration (Sep 10 2026, PM-authorized for internal synthetic UAT):** `origin/main` had advanced `eb4641f → 9219496` (+5 commits, all NY §396-b synthetic-performer Living Knowledge governance — one non-overlapping test-file hunk, clean re-rebase; new `likeness`-topic reviewer-eligible claim `CLAIM-NY-SYNTHETIC-PERFORMER-DISCLOSURE-001-v1` jurisdiction-scoped to New York now in `TOPIC_CLAIMS_FIXTURE`, so `likeness` is no longer an empty-coverage topic). 20-commit series re-rebased onto `9219496` (`git range-diff` all `=`), re-verified (`tsc` clean; full-suite failing set byte-identical to a fresh `9219496` baseline — zero new failures; `next build` exit 0), **fast-forward-merged to `main` `9219496 → 0413291` (no force)** and pushed. Production deployment confirmed by route topology (`POST .../reviewer-lk/research` → `401 {"error":"Unauthorized"}`, legacy `GET .../reviewer-lk` → `404`). **The authenticated production browser UAT (Phases 7–21), live classifier eval (Phase 16), and production audit-DB proof (Phase 17) were NOT performed — the Claude Chrome extension is not connected to this session and there is no admin credential / `ANTHROPIC_API_KEY` / audit-DB access here. These remain the gates before `CLOSED / PRODUCTION-PROVEN`.** | **HRR** (Human Reviewer Research) = the reviewer governed-research product capability inside the Commercial Assurance workspace; **GRI** (Governed Research Interface) = the reusable pattern *structured-or-NL intent → governed retrieval → applicability → Bounded Interpretation → consultative composition → provenance*. **HRR ≠ the Commercial Assurance Assessment** — the Human Reviewer keeps evidence evaluation / findings / control judgments / gaps / outcome / sign-off. V1: topic chips (0 model calls) + free-form questions (exactly 1 bounded **classify-only** structured LLM call — never generates the answer). **Both entry modes converge into ONE pipeline** — the topic path also flows through BI + HRR composition now (intentional evolution; authority spine unchanged: eligibility, `selectReviewerClaims`, deterministic applicability, audit-before-content). Reuses `lib/reviewer-lk/**` (`evaluateReviewerEligibility`, never `crc_eligible`), `lib/bounded-interpretation/**` via the **`BiIntent` + `BiResult` adaptation — SHIPPED (CAH-4G.1 + CAH-4G.3 + 3A)**, all proven byte-for-byte behavior-neutral for CRC (`userGoalsToBiIntents` + `lib/hrr/bi-adapters.ts`; NO synthetic `UserGoal` / `RetrievalResult` field; `RetrievalResult` structurally satisfies `BiResult` — the `applicability` field is optional — so CRC call sites are unchanged; for HRR `BiIntent.intent_text` = the fixed topic label, raw question never in BI). **Durable principle (CAH-4G.3A):** deterministic applicability is an upstream semantic input to BI — a proposition with `unresolved` applicability is `relevant_applicability_unresolved`, never `directly_relevant`, and Composition can never repair an overly strong BI result. `determination_declined` = the "should I approve this?" refusal (no yes/no, no invented topic). **Mixed-intent rule (frozen, corrected CAH-4G.1):** a single utterance may carry multiple semantic intents; the authority gate routes each independently — the assessment-decision intent is declined, and every explicitly-supported research clause survives at **its own scope** (usually `informational` → ordinary governed answer, NOT `determination_declined`); a prohibited intent never rewrites a permitted clause's scope. **Trust/grounding (frozen):** reviewer text is untrusted, grounds INTENT provenance only — every substantive proposition traces to grounding A–E (governed statement / applicability / BI output / fixed template / mechanical enumeration); a false premise is never echoed as fact. No server conversation state, no raw-question persistence; no automatic Linked CRC context. **CAH-4G.10 Slice A (CLOSED / PRODUCTION-PROVEN — `main` = `4b9ee5a`, CAH-4G.10C):** an append-only **client-only VISIBLE** research thread — each free-form question still classified independently from its own text alone (no `prior_context`); still no server state, no persistence, no transcript-to-LLM. Slice B adds a bounded enum-only follow-up referent (NOT STARTED / not authorized). **Audit (frozen):** reuse `access_kind='lk_research'` + one additive nullable migration (`research_mode`/`resolved_topics`/`question_intent`/`governed_claim_ids`); raw question never stored; audit-before-content preserved + strengthened. **No CRC change — CRC stays free-form during the pilot deliberately.** Auth: `is_admin` OK for internal pilot; dedicated grant before broader rollout. `ADR-002`: a natural-language research interface is intent entry, not an answer authority. Topic coverage: `copyright_ownership` (`CLAIM-COPY-004-v1`) + `copyrightability` (`CLAIM-COPY-001/002/003-v1`) have reviewer-eligible governed coverage; `likeness` gained reviewer-eligible coverage on `origin/main` at `9219496` (`CLAIM-NY-SYNTHETIC-PERFORMER-DISCLOSURE-001-v1`, jurisdiction-scoped to New York — for a non-NY submission it resolves applicability-unmet → `does_not_apply[]`, not `outside_current_coverage`). Genuinely open: `HRR_MAX_RESOLVED_TOPICS` (rec. 2 + a bounded experiment). |
| **PRD: CAH-4F Reviewer Resources** | `08_Platform/prds/PRD_CAH_4F_REVIEWER_RESOURCES.md` | CAH-4F = INTEGRATED/DEPLOYED (`ed3333e`); **CAH-4F.1 = INTEGRATED / PRODUCTION DEPLOYED** (`origin/main` = `b0e067f`, rebased onto `9203162` + fast-forwarded 2026-09-10; Vercel `si8-creator-portal` Production build `success`; shell IA confirmed operational during the CAH-4F.2 production UAT — dedicated §16 script not run separately, no separate formal CAH-4F.1 closure decision); **CAH-4F.2 Contextual Inspector Coordination = CLOSED / PRODUCTION-PROVEN** (`origin/main` = `f29b0fb`, feat `4f1969d`, rebased onto `74a8f63` + fast-forwarded 2026-09-10; Vercel `si8-creator-portal` Production build `success` on `app.superimmersive8.com`; PM/user authenticated visual UAT PASS — §17a) — production UAT of 4F.1 confirmed the shell but exposed four-region density at ~1440px; the contextual right rail now holds one surface at a time at adjacent widths via a layout-only `workspace-layout-context` boolean; `WorkbookClient` CSS-hides (never unmounts) its own Guidance/Submission/Evidence aside while Reviewer Resources holds the rail; `rightTab` stays Workbook-owned; UX mechanism, not an authority boundary; audit path unchanged (CAH-4E already proved the `lk_research` contract). Non-blocking polish observed: empty "Resolved tools —" could later be omitted | One product spec, three layout milestones. **CAH-4F** shipped the Reviewer Resources concept + LK Context Semantics (plain-English topic labels/chips, explicit Applicability heading, neutral status, "Context used for this look-up", proposition-first order, progressive provenance, raw `crc_publication_scope` prose + `crc_eligible` removed, no silent promotion) as a **grouped top-of-page block** — semantics validated + retained, but the top-of-page *layout* did not deliver the primary/secondary hierarchy in production. **CAH-4F.1 — Reviewer Workspace Shell** supersedes that layout: a generic client `ReviewerShell` owns the page frame (`ASSESSMENT NAV \| WORK SURFACE \| REVIEWER RESOURCES` closable inspector; adjacent column ≥1440px / overlay drawer below; tabs Living Knowledge + Linked CRC Context; CRC always-a-tab with neutral empty state when unlinked). `WorkbookClient` change = one line (`h-screen`→`h-full`); never remounted by inspector open/close. No route/selector/eligibility/applicability/audit/schema/migration change. Full Jest failure set byte-identical to parent `ed3333e` (20 pre-existing unrelated), +35 new passing reviewer/shell tests, zero new failures; `tsc` clean; `next build` exit 0. Browser tooling unavailable in the build env → deterministic per-viewport layout evidence + SSR DOM inspection instead of screenshots. Integrated to `origin/main` = `b0e067f` and Vercel-deployed to `si8-creator-portal` Production (`app.superimmersive8.com`) on 2026-09-10 after fresh re-validation from the rebased tree (14 reviewer suites / 301 pass, `tsc` clean, `next build` exit 0, full-suite failing set byte-identical to the `9203162` baseline, +35 new passing); the three-region shell + closable inspector + LK / Linked-CRC tab separation + "no stacked layout" were exercised and confirmed operational in production during the 2026-09-10 CAH-4F.2 UAT (§17a), though a dedicated §16 CAH-4F.1 UAT run and a separate formal CAH-4F.1 closure decision were not made. §13 OQ-1 re-resolved to the inspector. Companion decision: `lib/reviewer-lk/ADR-001-...`. Sep 10, 2026. |
| **CRC Publication Policy** | `06_Operations/institutional-knowledge/notebook/CRC-PUBLICATION-POLICY.md` | ACTIVE | The judgment layer between `Status = Verified` (a fact is true) and `CRC-Eligible = Yes` (SI8 will let that fact go out through CRC with no human reviewing the specific moment it's said). Six principles incl. claim-level approval, narrow-before-withholding (except Principle 3 sensitivity, which is a hard gate no amount of verification resolves), and the "Publication Test" (would this be comfortable quoted back by a prospect's legal team). Not one of the Living Notebook's four canonical documents — an adjacent policy doc. Frozen Aug 5, 2026. |
| **Commercial Assurance Intelligence — Vision v0.1** | `06_Operations/institutional-knowledge/COMMERCIAL-ASSURANCE-INTELLIGENCE-VISION-v0.1.md` | ACTIVE | Non-committal strategic vision — not a roadmap. Three-stage arc (Living Notebook → Knowledge Base → Commercial Assurance Intelligence); why evidence-capture (C2PA, prompt-logging, on-chain notarization) and evidence-*interpretation* are complementary, not competitive; SI8's moat framed as accumulated judgment, not document templates. Aug 1, 2026. |
| **Living Notebook** | `06_Operations/institutional-knowledge/notebook/` | ACTIVE — v1.0 architecture frozen Aug 2, 2026 | Four documents implementing the PRD above — Platform Rights Matrix, SI8 Positions, Edge Cases, Pending Questions. Seeded Jul 30 – Aug 1, 2026 with real entries, then reclassified Aug 1-2 after external review (fact/interpretation/policy split on every Position and Matrix row; Anchor Film corroboration moved into POS-001 as source evidence, not a standalone Edge Case; AI Wave panel Q&A moved to Pending Questions PQ-004/PQ-005, not treated as resolved precedent). Only EC-001 (Cloud World Domain H case) remains in Edge Cases. Aug 5-6, 2026: Platform Rights Matrix verified against primary sources across an 8-platform tranche (Runway/Kling/Midjourney/Pika/ElevenLabs reviewed and CRC-published per claim; Firefly/Veo still Needs Reverification; Gemini split into separate API/Consumer App rows, not yet reviewed) — see Matrix Learnings below. |
| **Matrix Learnings** | `06_Operations/institutional-knowledge/notebook/MATRIX-LEARNINGS.md` | ACTIVE | Schema-validation log from the 8-platform verification pass — tests whether the Matrix schema itself holds up, not just whether platforms got populated. Real findings: one disproven secondary-source misconception (Runway's Free tier is not non-commercial) that recurred and resolved oppositely for two other platforms; 7 distinct primary-source access-friction patterns; the compound-Status-field gap that led to the Status convention above. Not one of the Living Notebook's four canonical documents. Aug 5-6, 2026. |
...
## ACTIVE STRATEGIC HYPOTHESIS (Not Yet Validated — Aug 29, 2026)

**Status: WORKING HYPOTHESIS under active validation, not a product decision.** Recorded here per the Chief of Staff Operating Standards (below) so future sessions know what is currently being tested, why, and what must not yet be built or institutionalized on the strength of it. This section does not change CRC's frozen PRD, Commercial Assurance's current product definition, `PRICING-STRATEGY-v3.0.md`, or `TECHNICAL_ARCHITECTURE.md` — none of those are authorized to be reinterpreted through this lens yet.

**The central hypothesis:** SI8's durable strategic core may eventually be a project-state/evidence-aware **Commercial Readiness Decision Engine** ("DE" — a working term only, not approved terminology) for AI-generated media, rather than an artifact-compliance scanner. The insight under validation: *artifact compliance is not necessarily the same thing as project commercial readiness* — two identical media files can have different correct commercial-readiness conclusions depending on facts invisible in the pixels/audio (provider/tier, source-asset rights, talent/likeness/voice consent, music licensing, territory, intended use, evidence present or missing). This emerged from SI8's own CRC/Commercial Assurance/Living Knowledge work, Creative Producer and Anchor Film/Alice production-evidence conversations, competitor research (ZebraTruth, Creation Rights, Cleo Legal Data — not to be confused with Clio), and independent strategic review/falsification.

**Provisional three-surface model (working analysis frame, not official product names):** (1) **Living Knowledge** — governed decision-usable knowledge; new working hypothesis that SI8 may not need to own every underlying legal/source corpus and could license/integrate external infrastructure (e.g. Cleo Legal Data, WIPO, provider docs), with SI8's proprietary value sitting more in governance/decision methodology than raw data ownership. (2) **Commercial Readiness Decision Engine ("DE")** — potential capability consuming project state + evidence state + governed knowledge to produce a bounded commercial-readiness decision, rather than only an artifact+jurisdiction compliance scan. (3) **CRC** — under this hypothesis, potentially one interface into the underlying decision capability (conversational acquisition, missing-info identification, education, explanation, escalation toward Commercial Assurance) — **CRC's current, frozen product definition is unchanged by this hypothesis and remains authoritative until a separate, explicit decision changes it.**

**Commercial Assurance** is not one of the three surfaces — it remains the higher-assurance, human-reviewed product. The boundary "CRC educates; Commercial Assurance provides higher-assurance human review" is preserved. New hypothesis under evaluation: Commercial Assurance could eventually run over the same underlying Decision Engine, with machine-executed analysis where reliably determinable and human review concentrated on exceptions/judgment/ambiguity — never "humans manually redo everything the machine already resolved."

**Lifecycle hypothesis:** PLAN → RECORD → DECIDE/ASSURE. Pre-production, SI8 could assess whether an intended AI production plan is commercially supportable and what evidence it will need. Production/evidence-capture is explicitly NOT something SI8 intends to own (Creation Rights, Anchor Film, customer systems, AI platforms, DAMs, provenance infra could capture it). Pre-delivery, SI8 could evaluate actual state + evidence against what was planned. This is a working hypothesis, not a roadmap commitment.

**Potential future API surface:** if validated, the Decision Engine could eventually be exposed to partners (Creation Rights, Anchor, agencies, enterprise production systems) who submit structured project state and receive machine-resolved issues, missing-evidence flags, and human-review escalation — **not authorized to be designed or built now.**

**Competitive read (working, not settled):** ZebraTruth is treated as the highest-priority direct competitor to the Surface-2 thesis — a serious automated compliance Decision Engine (not a "simple pixel scanner"), currently public-documentation-oriented around artifact + distribution context → PUBLISH/HOLD/BLOCK, without yet publicly demonstrating equivalent structured project-state/evidence reasoning. This distinction must be benchmarked, not assumed, and SI8 should not assume ZebraTruth lacks undisclosed private/enterprise capability. Creation Rights could be a partner (rich production-state capture SI8 evaluates) or a vertically-integrated competitor — not assumed either way. Cleo Legal Data is treated as a probable upstream data/infrastructure supplier, not a direct competitor.

**Buyer/user uncertainty (open, not resolved):** Creative Producers are NOT institutionalized as the economic buyer. Current CP outreach evidence shows the compliance-vs-readiness distinction is discussion-worthy for some CPs; it does not establish willingness to pay, purchase trigger, or that CP is the actual economic buyer rather than the operational user. Agencies/Business Affairs, brand legal/procurement, insurers/E&O brokers, and production platforms (e.g. Creation Rights) are additional untested buyer/gatekeeper hypotheses.

**Three independently-failable hypotheses this reduces to** (per independent strategic review): **(A) Category/capability** — does project/evidence state actually change commercially relevant decisions materially? **(B) Customer/demand** — who cares enough to pay or change behavior? **(C) Economics** — can enough of the process be machine-resolved for human review to stay economically scalable? The combined independent-review conclusion is explicitly neither "build the Decision Engine" nor "reject it" — it is that the thesis is conceptually credible enough to validate and has not yet earned an engineering commitment.

**Falsification conditions** (any of these should materially weaken the thesis): project/evidence state rarely changes commercially important decisions; ZebraTruth/Creation Rights/frontier general-purpose AI can already do equivalent project-state reasoning; customers/gatekeepers don't care about the distinction; structured-input friction is too high; human-review rate makes the economics unattractive; independent third-party decisioning doesn't beat self-attestation/internal legal review/contractual indemnification in practice.

**What must NOT be built or institutionalized on this hypothesis yet:** a generalized API; a rewritten `TECHNICAL_ARCHITECTURE.md`; updated pricing in `PRICING-STRATEGY-v3.0.md`; any change to `PRD_CRC_v1.0.md`, `PRD_LIVING_NOTEBOOK.md`, `PRD_LIVING_KNOWLEDGE_SOURCE_INPUTS_v0.1.md`, `PRD_ASSESSMENT_SERVICE_v1.0.md`, or `CRC_CURRENT_STATE.md`; any code/schema implementation. See EXECUTION GAPS §3s below for the current validation workstreams, and `BUSINESS_PLAN_v4.md`'s own "Strategic Product Direction Under Validation" section for the business-plan-level framing.

**The one thing actually decided (DECISION, not hypothesis):** validate the Decision Engine thesis (capability, demand, economics, in parallel) before committing to architecture or API implementation. Everything else above remains WORKING HYPOTHESIS or OPEN QUESTION.
...
**Document & File Naming Conventions (reviewed Aug 6, 2026 — folder-based pattern, not a single global rule):**
- `08_Platform/prds/` — underscore style, `PRD_XXX.md`. Product-requirements documents only: what the product does, its philosophy, normative behavior. Not implementation detail.
- `06_Operations/institutional-knowledge/notebook/` and sibling institutional-knowledge docs — hyphen style, `XXX-YYY.md` (`PLATFORM-RIGHTS-MATRIX.md`, `SI8-POSITIONS.md`, `CRC-PUBLICATION-POLICY.md`, `MATRIX-LEARNINGS.md`). This pattern already holds consistently and doesn't need to change.
- `08_Platform/implementation/` — the one place this got inconsistent: `RECORD_FORM_IMPL.md`/`CERT_FORM_IMPL.md` use an `_IMPL` suffix, while `4_WEEK_SPRINT_PLAN.md` and `REPORT_BINDING_STAGING_CHECKLIST.md` don't follow any shared pattern. Not worth renaming existing files retroactively (real risk of breaking cross-references for low value) — but going forward, two distinct suffixes for two distinct document types: **`_IMPL.md`** = engineering plan for a feature already specified elsewhere (build order, phases, known bugs, migration/test plan — e.g. RecordForm, CertForm). **`_ARCHITECTURE.md`** = internal design document defining how a subsystem represents and processes state, when the *what* is already normatively specified in a frozen PRD (e.g. `INTERVIEW_ENGINE_ARCHITECTURE.md` defines how the Interview Engine works internally; `PRD_CRC_v1.0.md` already defines what it must do).
