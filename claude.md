...
| **PRD: CAH-4G HRR / Governed Research Interface** | `08_Platform/prds/PRD_CAH_4G_HRR.md` · design `08_Platform/implementation/HRR_GRI_TECHNICAL_DESIGN.md` · decision `08_Platform/app/lib/reviewer-lk/ADR-002-governed-research-interface.md` | **`DESIGNED / NOT IMPLEMENTED`** (Sep 10, 2026) | **HRR** (Human Reviewer Research) = the reviewer governed-research product capability inside the Commercial Assurance workspace; **GRI** (Governed Research Interface) = the reusable pattern *structured-or-NL intent → governed retrieval → applicability → Bounded Interpretation → consultative composition → provenance*. **HRR ≠ the Commercial Assurance Assessment** — the Human Reviewer keeps evidence evaluation / findings / control judgments / gaps / outcome / sign-off. V1: topic chips unchanged (0 model calls) + free-form questions (exactly 1 bounded **classify-only** structured LLM call — never generates the answer). Reuses `lib/reviewer-lk/**` retrieval/eligibility (`evaluateReviewerEligibility`, never `crc_eligible`), `lib/bounded-interpretation/**` (semantic ceiling; `determination_declined` = the "should I approve this?" refusal — no yes/no, no invented topic), the `anthropic-extractor.ts` structured-output pattern. Single-turn; no automatic Linked CRC context; no synthetic `UserGoal`s (honest `BiIntent` adaptation boundary); reuse `access_kind='lk_research'` (audit contract deferred). **No CRC change — CRC stays free-form during the pilot deliberately.** `ADR-002`: a natural-language research interface is intent entry, not an answer authority. Topic-coverage note: `copyright_ownership` (`CLAIM-COPY-004-v1`) + `copyrightability` (`CLAIM-COPY-001/002/003-v1`) DO have reviewer-eligible governed coverage at `origin/main`=`238670c` (the earlier "zero coverage" claim read a stale `bounded-interpretation/types.ts` comment); the one real gap is `likeness` (0 topic claims). |
