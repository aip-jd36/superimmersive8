...
| **REVIEWER RESOURCES — Architecture** | `08_Platform/implementation/REVIEWER_RESOURCES_ARCHITECTURE.md` | ACTIVE | The as-built internal design of the reviewer-side surfaces CAH-4B…4F (CAH-4B reviewer CRC context, CAH-4C transcript access, CAH-4E Reviewer Living Knowledge, CAH-4F Reviewer Resources presentation — component/data flow, system boundaries, retrieval/applicability/UI-projection/audit contracts, firewall test map). §7 is the CAH-4F as-built file table. As-built basis `9fa6d1c` + the CAH-4F implementation commit. Sep 9, 2026. |
| **PRD: CAH-4F Reviewer Resources** | `08_Platform/prds/PRD_CAH_4F_REVIEWER_RESOURCES.md` | INTEGRATED / DEPLOYED — PRODUCTION UAT INCOMPLETE (`origin/main` = `d19b678`, Sep 9, 2026) | Product contract for the CAH-4F milestone, built as a reviewer-page **presentation** change only: a `ReviewerResources` server-component container grouping Living Knowledge + Linked CRC Context beside the workbook (OQ-1 → §8.3 stacked-grouped fallback; `WorkbookClient.tsx` untouched); LK Context Semantics (plain-English topic labels, explicit Applicability heading, neutral styling, "Context used for this look-up", proposition-first card order, progressive provenance disclosure, raw `crc_publication_scope` prose + `crc_eligible` removed from the reviewer view, no silent promotion to evidence/findings). No route/selector/eligibility/applicability/audit/schema/migration change. Integrated by fast-forward `e2b736d..d19b678` (rebased onto the 4 upstream LK-TRIAL-11 commits, zero file overlap, patch-ids identical). Full suite at integrated HEAD = 20 pre-existing unrelated failing suites, **identical set to the integration base** `e2b736d`, +41 new passing CAH-4F tests, zero new failures. §13 open questions all resolved; §15 UAT script ready (operator-gated — needs an authenticated reviewer session). Explicit CAH-4G (conversational LK) future boundary §11. Companion decision: `lib/reviewer-lk/ADR-001-reviewer-resources-authority-boundary.md`. Sep 9, 2026. |
