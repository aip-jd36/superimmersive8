...
| P001 | Sofia Yan | Numbers Protocol (Capture) | partner | Call Taken | 2026-07-01 | In-person Taipei meeting Jul 1. KEY FINDING: old Capture (upload-based API signing) is NOT on C2PA Trust List under 2024-25 requirements — only Proof Snap (mobile camera) qualifies. Upload-based content = C2PA metadata embedded but signer not Trust List-certified. ERC-7053 on-chain registration still fully functional regardless. Numbers Protocol confirmed infra-only — no plans for compliance/legal review layer. SI8's evidence package concept resonated ("有点像SOC2 audit"). Sofia offered API access directly. Both confirmed for coffee follow-up. Call notes: `03_Sales/call-notes/CALL-2026-07-01-P001-Sofia-Yan-Numbers-Protocol.md` | Email Sofia with partnership framing ideas (JD promised post-call). Request API access. Run test sign on a real video. Revise v4.1 claims — remove "C2PA Trust List certified" language. | 2026-07-07 | HIGH | partner — api test next |
